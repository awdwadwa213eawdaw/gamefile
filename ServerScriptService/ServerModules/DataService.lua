...
do -- Database Setup
	...
	Database.Objectives = data.Objectives
	Database.MoveToName = {}
	...
	for _, d in pairs(data.Movedex) do
		if d.id then
			Database.MoveToName[d.id] = d.name
		end
	end

	--	Database.ItemByNumber installed later

	Database.boardFns = data.boardFns
...
do -- Data Setup
	...
	local function copyGifData(id, id2, fn)
		for i=1, #keys do
			local key = keys[i]
			local gifData = deepcopy(data.GifData[key][id])
			if gifData then
				if fn then
					fn(gifData)
				end	
				data.GifData[key][id2] = gifData
			end	
		end	
	end

	-- Customs with new scale
	local customs = {
		{"Bidoof", "God", 2.5},
		{"Snorlax", "Chungus", 2.27},
		--{"Magikarp", "Big", 2.5},
	}

	for i, mon in pairs(customs) do
		local name, forme, scale, oldForme = unpack(mon)

		copyGifData(name..(oldForme and "-"..oldForme or ""), name.."-"..forme, function(gifData)
			gifData.scale = scale
		end)
	end

	local cryIds = { Audios.Cry1, Audios.Cry2, Audios.Cry3, Audios.Cry4,--// Cries 1-4
		...
		Audios.Cry15, Audios.Cry16, Audios.Cry17, Audios.Cry18 }--// Cries 15-17
...
--Colo Banlists
for _, file in pairs(storage.Data.ColoBanlist:GetChildren()) do
	local banlist = require(file)
...
local function completeBattle(num, ch, isRandomizer)
	...
	if b.TrainerClass == 'Eclipse Grunt' then
		b.IconId = 11614722548
		b.musicId = {Audios.EclipseGruntBattle1, Audios.EclipseGruntBattle2}
		b.musicVolume = 2
	end
...
function DataService.checkchunk(player, chunkid)
	...
	local chunktoevent = {
		...
		['chunk75'] = 'MeetTessBeach',
		['chunk76'] = 'MeetTessBeach',
		['chunk77'] = 'MeetTessBeach',
		['chunk78'] = 'MeetTessBeach',
		['chunk79'] = 'MeetTessBeach',
		['chunk80'] = 'vCrescent',
		['chunk81'] = 'vCrescent',
		['chunk82'] = 'vCrescent',
		['chunk83'] = 'vCrescent',
		['chunk84'] = 'vCrescent',
		['chunk85'] = 'PressSecurityButton',
		['chunk86'] = 'PressSecurityButton',
		['chunk87'] = 'PressSecurityButton',
		['chunk88'] = 'UnlockGenDoor',
		['chunk89'] = 'DefeatEclipseBase',
		['chunk90'] = 'DefeatEclipseBase',
	...
	if chunktoevent[chunkid] then
		if chunktoevent[chunkid]:sub(1, 3) == 'gym' then
			if not PlayerData.badges[tonumber(chunktoevent[chunkid]:sub(4, 4))] then
				...
				pcall(function() player:Kick("Please avoid exploiting. Further exploitation can and will result in a permanent ban. Make sure you didn't skip anything important to the story.") end)
				wait()
		...
		elseif not PlayerData.completedEvents[chunktoevent[chunkid]] then
			...
			pcall(function() player:Kick("Please avoid exploiting. Further exploitation can and will result in a permanent ban. There is a chance this might be a mistake, if you weren't exploiting, try going back and making sure you didn't skip anything important.") end)
			wait()
			pcall(function() player:Destroy() end)
			return false
		end
	elseif customchunktoevent[chunkid] then
		if not customchunktoevent[chunkid]['haspass'] then
			...
			pcall(function() player:Kick("Please avoid exploiting. Further exploitation can and will result in a permanent ban.") end)
			wait()
			pcall(function() player:Destroy() end)
			return false
		end
...
function DataService.fulfillRequest(player, ...)
	...
	for i, request in pairs({...}) do
		...
		if kind == 'Chunk' then
			...
			if PlayerData then
				if PlayerData.currentChunk then
					lastChunkData = data.Chunks[PlayerData.currentChunk]
				end
			...
			if not map then
				...
				if not baseMap then
					warn("Missing chunk in storage.MapChunks:", chunkId)
					return false
				end
				map = baseMap:Clone()
			end
			...
			spawn(function()
				if lastChunkData then
					if lastChunkData.isSafari and not d.isSafari then
						PlayerData:leaveSafari()
					elseif d.isSafari and not lastChunkData.isSafari then
						PlayerData:enterSafari()
					end	
				end	
			end)
			...
			--// Winter Event 2022 (Revamped 2016)
			--[[ if chunkId == 'chunk5' or chunkId == 'chunk11' or chunkId == 'chunk15' then
				local santa = map:FindFirstChild('Santa')
				local hour = game:GetService("Lighting"):GetMinutesAfterMidnight() / 60
				if santa and PlayerData then
					if PlayerData.completedEvents.BeatSanta or not PlayerData.completedEvents.LearnAboutSanta then
						santa:Destroy()
					else
			...
			local weather = (_f.Context == 'adventure' and _f.currentWeather)
			...
			v = {map = map, data = d, rooms = rooms, roomData = roomData, id = chunkId, battles = battles, 
				grassReplication = grassReplication, weather = weather--[[, oItems = obtainableItems]]}
