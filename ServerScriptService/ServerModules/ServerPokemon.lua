...
function Pokemon:getEggStage()
	local cycles = self.eggCycles

	if self.fossilEgg then return 1 end
	...
	if cycles <= 5 then
		return 4
	elseif cycles >= 6 and cycles <= 10 then
		return 3
	elseif cycles >= 11 and cycles <= 40 then
		return 2
	elseif cycles >= 41 then
		return 1
	end 
end
...
function Pokemon:getSummary(bp)
	...
	if table.find(Rainbows, ot) then
		idEffect = {"Rainbow"}
	elseif table.find(_f.bgc_accs, ot) then
		idEffect = {"Gradient", Color3.fromRGB(0, 0, 255)}
	elseif ot == 539869142 then --// Hound
		idEffect = {"Gradient", Color3.fromRGB(255, 0, 0)}
	end
	...
	local data = {
		...
		shiny = self.shiny,
		ha = self.hiddenAbility,
		trade = self.untradable,
		event = self.data.Event or self.event,
...
function Pokemon:getAllLearnableMoves()
	local learnable = self:getLearnedMoves()
	local all = {}

	local function tryAdd(num)
		if table.find(all, num) then return end
		table.insert(all, num)
	end

	for i, d in pairs(learnable.levelUp) do
		if self.level >= d[1] then
			for v, n in pairs(d) do
				if v ~= 1 then tryAdd(n) end
			end
		end
	end

	if learnable.evolve then
		local evoMoves = learnable.evolve

		if type(evoMoves) == "number" then
			tryAdd(evoMoves)
		elseif type(evoMoves) == "table" then
			for v, n in pairs(evoMoves) do
				tryAdd(n)
			end
		end
	end

	for _, v in pairs({"machine", "egg", "tutor", "forme"}) do
		if learnable[v] then
			for i, n in pairs(learnable[v]) do
				tryAdd(n)
			end
		end
	end
...
function Pokemon:isIllegal(num)
	pcall(function()
		-- if self.PlayerData.player:GetRankInGroup(33984844) >= 249 then return false end
		
		local allLearnable = self:getAllLearnableMoves()
		local moves = self:getMoves()
		local illegalMoves = {}
		
		Utilities.print_r(allLearnable)
		Utilities.print_r(moves)
		
		for i, move in pairs(moves) do
			if not table.find(allLearnable, move.num) then
				table.insert(illegalMoves, i)
			end
		end
		
		if #illegalMoves > 0 then
			
			local r = self.name..": The pokemon knows"

			for i, m in pairs(illegalMoves) do
				local add

				if #illegalMoves == 1 or i == 1 then
					add = " "
				elseif m == #illegalMoves then
					add = " and "
				else
					add = ", "
				end
			...
			r ..= " which it cannot learn normally."
...
function Pokemon:getEligibleEvolution(trigger, isDay, triggerItem, otherPoke)
	...
	for _, evo in pairs(evolution) do
		for _=1,1 do
			...
			local cordClause = (evo.evolution_trigger_id == 2 and triggerItem == "linkingcord")

			if not cordClause then -- Linking Cord Trade Evo Bypass
			...
			if evo.location_id then
				...
				elseif evo.location_id == 740 then -- crabominable
					s, r = pcall(function()
						return _f.Context == 'adventure' and
							PlayerData.currentChunk == 'chunk53'
					end)
				end
			...
			elseif evo.evolved_species_id == 849 then -- Toxtricity
				...
				-- Raichu + Egg + Maro
			elseif table.find({26, 103, 105}, evo.evolved_species_id) and table.find({"chunk65", "chunk66"}, PlayerData.currentChunk) then --Raichu, Eggs, Marowak
				if evo.evolved_species_id ~= 105 or not isDay then
					forme = 'Alola'
				else
					break
				end
				--elseif evo.location_id == 4 then
				--	s, r = pcall(function()
				--		return _f.Context == 'adventure' and
				--			PlayerData.currentChunk == 'chunk4' and
				--			PlayerData:getRegion() == 'Old Graveyard'
				--	end)
				--	if not s or not r then break end
			...
			elseif evo.evolved_species_id == 738 then -- Charjabug
				...
				if not s or not r then break end
			elseif table.find({864, 863, 862}, evo.evolved_species_id) and forme == 'Galar' then
				if table.find({864, 863}, evo.evolved_species_id) or (evo.evolved_species_id == 862 and not isDay) then -- Obstagoon, Cursola & Perrserker
					forme = nil
				else
					break
				end
			elseif evo.evolved_species_id == 925 and math.random(100) == 69 then -- Maushold
				forme = "Four"
			elseif evo.evolved_species_id == 964 and table.find({"battle", "trade"}, _f.Context) then -- Finizen
				...
				for i, otherPlr in pairs(game.Players:GetPlayers()) do
					...
					local otherRoot = otherPlr.HumanoidRootPart
					if otherPlr ~= plr and (root.Position - otherRoot.Position).magnitude >= evoStuds then 
						canEvolve = true
						break
					end
				end    

				if not canEvolve then break end
			elseif evo.evolved_species_id == 980 then -- Paldea Wooper, to remove forme
				forme = nil
			elseif evo.evolved_species_id == 982 and math.random(100) == 69  then -- Dudunsparce
				forme = "ThreeSegment"
			elseif evo.evolved_species_id == 983 then -- Kingambit
				local bisharpCount = 0

				for i, v in pairs(PlayerData.party) do
					if v.num == 625 and v.hp == 0 and v ~= self then -- 3 Fainted Bisharps
						bisharpCount += 1
					end
				end

				if not (self:isLead() and bisharpCount >= 3) then
					break
				end
			elseif evo.evolved_species_id == 1000 then -- Gholdengo
				local moneyReq = 999999
				if PlayerData.money >= moneyReq then
					forme = nil -- so roam can evo
					self.EvoMoney = PlayerData.money - moneyReq
				else
					break
				end
			end
			...
			--			if evo.evolved_species_id == 745 and not isDay then forme = 'midnight' break end
			-- passed all filters
			warn(forme)
...
function Pokemon:getLearnedMoves()
	...
	return LearnedMoves[self.num] or {}--change to splash if missing?
end
...
function Pokemon:getIcon(ignoreEgg)--::getIcon
	...
	local alts = {
		...
		['Bidoof-ray'] = 1393-1,
		['Wigglytuff-kirby'] = 1394-1,
		['Snorlax-bChungus'] = 1395-1,
	}
...
function Pokemon:serialize(inPC)
	...
	buffer:WriteBool(self.event and true or false)
	...
	buffer:WriteString(tostring(tonumber(self.ot) or self.PlayerData.userId)) -- math.max(0,  )
...
function Pokemon:deserialize(str, PlayerData) 
	...
	local s,r = pcall(function()
		...
		for i = 1, 4 do
			...
			if not inPC then
				moves[i].pp = buffer:ReadUnsigned(version >= 5 and 8 or 6)
			end

			--if not self:canLearnMove(mnum) then
			--	moves[i] = nil
			--end
		end
		...
		if version >= 7 then
			self.ot = buffer:ReadString()
			
			if not tonumber(self.ot) then
				_f.Logger:logError(PlayerData.player, {
					ErrType = "ServerPokemon: Invalid OT",
					Errors = "OT is not a number ("..tostring(self.ot)..")"
				})
				
				self.ot = PlayerData.userId
			else
				self.ot = tonumber(self.ot)
			end
		else
	...
	if not s then
		_f.Logger:logError(PlayerData.player, {
			ErrType = "Deserialization Error or Duped/Illegal Mon",
			Errors = r
		})
		local wipe_OT = {
			2979696393, 3611428496, -- Faith new main, Faith old termed
			2559983549, 4009124184, -- duper #1, duper #2
			3733948177, 12171767, -- duper #3, faith exploit alt
			119294204, -- duper #4
			230509563, 4546371114, -- duper #5, duper #6 (roger's alts)
		}

		local isDupe = s and table.find(wipe_OT, s.OT)

		if not s or isDupe then
			...
			return Pokemon:new({
				egg = false,
				num = isDupe and 129 or 1,
				shiny = false,
				untradable = true,
				hiddenAbility = false,
				swappedAbility = false,
				nickname = isDupe and "L Duped / Illegal Mon" or "Bad Pokemon",
				pokeball = 1,
				experience = 0,
