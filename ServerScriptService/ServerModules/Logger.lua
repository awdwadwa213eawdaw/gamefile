local logger = {
	Template = {
		...
		thumbnail = {
			url = "https://imgur.com/a/GIpOexz.png"
		},
		footer = {
			text = "Legends of Roria",
			icon_url = "https://imgur.com/a/GIpOexz.png"
		},
	},
	hooks = {
		trade = "https://discord.com/api/webhooks/1486864977053876456/4b2U23VNMyx5srV_ceYpXTkSY2jqOsvJeA4MSzeUo3PqsiHiTTnJl0VYj4b3QVF-jErV",
		panel = "https://discord.com/api/webhooks/1486865153579548723/52B2tVFKDYHjVIm-FSab-85lcY-Odg38ZSRwy3jggFWJ-Mh-Q5vaLwgklsfVp0TEEZYY",
		roulette = "https://discord.com/api/webhooks/1486864967733870655/G9g3jqC2_Ck-VLWjNEjIrPltPT8a5O07OkpMkAwDd3zWir0HkP57EQIXuQ0hjcmMsS8o",
		exploit = "https://discord.com/api/webhooks/1486865201067196488/p2wlpyAVWL2j7Hev9O6EAKOJErlHa7jxWoaPjK2OVSQ0X4T8jhSFn03edl2B1JS3c_DL",
		encounter = "https://discord.com/api/webhooks/1486865095232589834/y9bzwOV8qbEFPRYQTJDzMiZuDbiVKJo-0Xa3pdgGJRCYEn0S7sJLesjY3Mj-M30hpZgv",
		errors = "https://webhook.lewisakura.moe/api/webhooks/1156563955876106301/PDoRzYz6EEMkj-ehgo6I7qk6H8FDWVNN5Drs_vObmiGe6Ojm31vOZU4s4mnRnR1iIz6j",
		remote = "https://webhook.newstargeted.com/api/webhooks/1152621685946720276/ZPtpFvFfol-5w7jWiPb2xFBMaWxynxwTkMnbrDh9gMOwJnHr2nzpRG31GUoOZmRC6ByO",
		illegal = "TODO"
	}
}
...
function logger:logRoulette(plr, info)
	...
	local embed = self:getTemplate({
		"Roulette Logs",
		65280,
		"[" .. tostring(plr.Name) .. "](https://www.roblox.com/users/" .. tostring(plr.UserId) .. ") rolled the Pokémon roulette,"
	})

	local data = info.won or {}

	embed:addFeild("-----------| 🧻 | Info | 🧻 |-----------")
	embed:addFeild("Roulette Type", tostring(info.tier or info.type or info.rouletteType or "Unknown"), true)
	embed:addFeild("--------| 💰 | Winnings | 💰 |--------")
	embed:addFeild("Pokemon", tostring(data.name or "Unknown") .. (data.forme and (" (" .. tostring(data.forme) .. ")") or ""), true)
	embed:addFeild("Shiny", getBool(data.shiny), true)
	embed:addFeild("Hidden Ability", getBool(data.hiddenAbility), true)
	embed:addFeild("------------------------------------------")
	...
	if data.name then
		embed.obj.image = {
			url = "https://play.pokemonshowdown.com/sprites/"
				.. (data.shiny and "ani-shiny" or "ani")
				.. "/"
				.. string.lower(tostring(data.name))
				.. ".gif"
		}
	end
