	...
	function Battle:getAbility(ability)
		if not ability or type(ability) == 'string' then
			...
			if not ability.gen then
				local num = tonumber(ability.num) or 0
				if     num >= 165 then ability.gen = 6
				elseif num >= 124 then ability.gen = 5
				elseif num >= 77  then ability.gen = 4
				elseif num >= 1   then ability.gen = 3
				else                   ability.gen = 0
				end
			end
		end
