...
local MegaColors = {
	...
	LopunnyE    = {{'Brown','Beige','Carnation pink'}},
...
return function(Battle)
	...
	function Battle:canMegaEvo(pokemon)
		...
		if item.id == 'sceptilitec' and not table.find(cformes, pokemon.set.forme) then
			--print('wrong sceptile')
			return false
		elseif item.id == 'sceptilite' and table.find(cformes, pokemon.set.forme) then
			return false
		end

		-- special condition: only lopunnite easter can use lopunnitee
		if item.id == 'lopunnitee' and pokemon.set.forme ~= 'Easter' then
			--print('wrong lopunny')
			return false
		elseif item.id == 'lopunnite' and pokemon.set.forme == 'Easter' then
			return false
		end
	...
	function Battle:getMaxMove(move, pokemon) 
		...
		local maxMove = self:getMove(self:findIn(move.name, self.gmaxMoveTable, pokemon))--need to change
		local movename = move.name
		if (move.id == 'struggle') then 
			return move --Struggle always returns struggle
	...
	function Battle:canMaxMove(pokemon)
		...
		if pokemon.side.isTwoPlayerSide then 
			--if not pokemon.side.MaxmoveAdornment[pokemon.teamn] then return end
			return -- Don't allow dmax in 2v2s
		else
			if not pokemon.side.dynaAdornment then return end --currently using zpowerring as place holder item
		end	
	...
	function Battle:getZMove(move, pokemon, skipChecks)
		storage.Models.Zpoke.Value = move.type
		local item = pokemon:getItem()
