...
BattlePokemon = class({
	...
	self.statusData = {}
	if (self.species == 'Eternamax-Eternatus') then
	...
	if set.gigantamax and not self.canMegaEvo and not self.canZMove then self.gigantamax = true end
	self.maxMoves = self.battle:canMaxMove(self)
	self.canDynamax = self.maxMoves and true or false--(Disabled)
	self.currentDyna = false 
	if (self.species == 'Eternamax-Eternatus') then
		self.currentDyna = true 
	end
...
function BattlePokemon:maxMoveDisabled(baseMove)
	baseMove =  self.battle:getMove(baseMove)
	if self:getMoveData(baseMove.id) and not (self:getMoveData(baseMove.id).pp) then return true end
	if (baseMove.category == 'Status' and self:hasItem('assaultvest')) then
		return (self.volatiles['taunt'] and true or false)
	end
end
function BattlePokemon:getDynamaxRequest(skipChecks) 	
	if not (self.canDynamax or self.currentDyna) then return false end
	if (self.baseTemplate.isMega or self.baseTemplate.isPrimal or self:getItem().zMove or self.canMegaEvo) then
		return false
	end
	-- Some pokemon species are unable to dynamaxin
	if (self.baseTemplate.cannotDynamax) then 
		return false -- Zacian Zamazenta Eternatus (although Eternatus is dealt diffrently)
	end
	...
	for i, v in pairs(self.baseMoves) do
		local move = self.battle:getMove(toId(self.baseMoves[i])) or ''
		local maxMove = self.battle:getMaxMove(move, self)
		if (maxMove) then
			if (self:maxMoveDisabled(move)) then --Aparently They can be disabled (add check later in battleengine)
...
function BattlePokemon:isAlly(pokemon)
	return pokemon and pokemon ~= null and (self.side == pokemon.side)
end
function BattlePokemon:isAdjacent(pokemon2)
	if self.fainted or pokemon2.fainted then return end
	if self.side == pokemon2.side then return math.abs(self.position - pokemon2.position) == 1 end
	return math.abs(self.position + pokemon2.position - 1 - #self.side.active) <= 1
end
function BattlePokemon:updateSpeed()
	self.speed = self:getActionSpeed()
end
function BattlePokemon:getActionSpeed() -- Infrared carrying the BattleEngine because nobody else can add updated logic apparently
	local speed = self:getStat('spe')
	if self.battle:getPseudoWeather('trickroom') then
		speed = 10000 - speed
	end
	--print("Trick Room?", speed, trunc(speed, 13))
	return trunc(speed, 13)
end
function BattlePokemon:getUndynamaxedHP(amount) 
...
function BattlePokemon:getRequestData() -- for pokemon in request.active (for request.side, see BattleSide:getData())
	...
	--Allows for use in doubles/2v2s should remove the lock
	--[[
	if self.canZMove then
		for _, specialMove in pairs(self.canZMove) do
			data.ZMoves[#data.moves+1] = {move=specialMove, id=toId(specialMove)}
		end
	end
	if (self.maxMoves) then
		for _, specialMove in pairs(self.maxMoves) do
			data.maxMoves[#data.moves+1] = {move=specialMove, id=toId(specialMove)}
		end
	end
	--]]
	--	print(data.moves)
	for _, m in pairs(data.moves) do
		...
		m.category = self.battle:getMove(m.id).category
	end
	...
	--data.format = self.battle:getEffect(self.battle.format)
	---self.battle:call((self.battle:getEffect(self.battle.format).onBegin or self.battle:getEffect('none').onBegin))
	...
	if not lockedMove then --Cannot do these actions if the move is locked
		if self.canMegaEvo then
			data.canMegaEvo = true 
		end
		if self.canZMove then
			data.canZMove = self.canZMove
		end
		if self.canDynamax and self:getDynamaxRequest() then
			data.canDynamax = true
			data.maxMoves = self:getDynamaxRequest().maxMoves or self.maxMoves
		end
		if (self.canDynamax and self.gigantamax) then
			data.canDynamax = false
			data.gigantamax = true
		end
		if (self.currentDyna) then
			data.currentDyna = self.currentDyna
			pcall(function()
				data.maxMoves = self:getDynamaxRequest().maxMoves or self.maxMoves
			end)
		end
	...
	data.hpType = self.hpType or "Dark"
	data.forme = self.baseTemplate.forme
	data.types = self.baseTemplate.types
	data.ability = self.ability.name

	return data
end
-- Random Battles
function BattlePokemon:randomBattleData()
	local data = {
		name = self.name,
		species = self.species,
		icon = self.template.icon - 1,
		shiny = self.shiny,
		level = self.level,
		hp = self.hp,
		maxhp = self.maxhp,
		status = self.status,
		itemIcon = (self:getItem().icon or self:getItem().num),
		gender = self.gender,
		bindex = self.index,
		hiddenAbility = self.baseTemplate.hiddenAbility,
		hashiddenAbility = not Not(self.baseTemplate.hiddenAbility),
		gigantamax = self.gigantamax,
	}
	return data
end
function BattlePokemon:randomBattleSummary()
	local ballIcon
	pcall(function()
		local ball = _f.Database.ItemById[_f.ServerPokemon:getPokeBall(self.ball)]
	...
	for i, move in pairs(moves) do
		local moveData = deepcopy(_f.Database.MoveById[move.id])
	...
	local spriteId = self.species .. (self.forme or '')
	local spriteKind = self.shiny and '_SHINY_FRONT' or '_FRONT'

	local function statTable(t, hpIncluded)
		if not t then return end
		local keys = hpIncluded and {'hp', 'atk', 'def', 'spa', 'spd', 'spe'} or {'atk', 'def', 'spa', 'spd', 'spe'}
		local extra = {}
	...
	local data = {
		...
		hp = self.hp,
		maxhp = self.maxhp,
		stats = statTable(self.stats),
		status = self.status,
		itemName = self:getItem().name,
		gender = self.gender,
		bindex = self.index,
		nature = self.set.natureId,
		types = self.template.types,
		moves = moves,
		sprite = self.frontSpriteOverride or _f.DataService.fulfillRequest(nil, {'GifData', spriteKind, spriteId, self.gender=='F'}),
		id = 0,
		desc = "Random Pokemon",
		abilityName = self:getAbility().name,
		hiddenAbilityName = self.baseTemplate.hiddenAbility,
		hashiddenAbility = not Not(self.baseTemplate.hiddenAbility),
		gigantamax = self.gigantamax,

		evs = statTable(self.evs, true),
		ivs = statTable(self.ivs, true),
		bss = self.baseStatOverride or self.baseTemplate.baseStats,
...
function BattlePokemon:getEnabledMoves()
	local enabledMoves = {}
	for i, move in ipairs(self:getMoves()) do
		if not move.disabled and (not move.pp or move.pp > 0) then
