...
local function shutdown()
	...
	players.PlayerAdded:connect(function(p)
		wait()
		p:Kick('The game has just updated, please rejoin to play!! :)')
	end)
	for _, p in pairs(players:GetChildren()) do
		pcall(function() p:Kick('The game has just updated, please rejoin to play!! :)') end)
	end
end

local SecondaryDatastore = _f.FirebaseService
local banlist = SecondaryDatastore:GetFirebase('Banned')
...
local function globalNotificationReceived(n)
	...
	elseif n.kind == 'Ban' then
		...
		for _, player in pairs(players:GetChildren()) do
			...
			if player.Name == name then
				pcall(function() player:Kick("You've been permanently banned from Legends of Roria due to exploiting, cheating, or rule-breaking. You may also be blacklisted for external reasons, such as association with exploiters, mass-duping, and other heavy infractions.") end)
				wait()
				pcall(function() player:Kick("You've been permanently banned from Legends of Roria due to exploiting, cheating, or rule-breaking. You may also be blacklisted for external reasons, such as association with exploiters, mass-duping, and other heavy infractions.") end)
	...
	elseif n.kind == 'WeatherUpdate' then
		_f.Network:postAll('weatherChange', {
			End={{''}, _f.currentWeather},
			Start={{''},n.weather},
			StartNotif={Poke={name='???',icon=2001},
				regionName='???',
				weatherKind=n.weather
			}})
		_f.currentWeather = n.weather
	end
end
...
---------------------------------------------------------------------------------------------
local stores = _f.SecondaryDatastore
