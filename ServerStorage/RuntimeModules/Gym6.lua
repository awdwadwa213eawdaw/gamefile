...
--	for _ = 1, 3 do pcall(function() map.BuildBridge:Destroy() end) end
	bridges = {}
	currentBridge = nil
...
--				if region:CastPoint(f.Position) then
			...
			if nFruitInBasket ~= currentFruitCount then
				...
				if nFruitInBasket == 5 then
					pcall(function() map.LeaderGoalNub:Destroy() end)
...
function gym6:deactivate()
	...
	pcall(function() carrying:Destroy() end)
	...
	pcall(function() for _, d in pairs(dots) do d:Destroy() end end)
	pcall(function() for _, d in pairs( dq ) do d:Destroy() end end)
	dots, dq = {}, {}
	
	-- release de-parented objects
	if type(bridges) == 'table' then
		for _, b in pairs(bridges) do
			pcall(function() b.bridge:Destroy() end)
		end
	end
end
