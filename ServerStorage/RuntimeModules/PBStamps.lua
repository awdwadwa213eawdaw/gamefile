...
for i = 0, 4 do
	styleIcons[i+1] = create 'ImageLabel' {
		...
		Image = 'rbxassetid://13050538949', -- Stamp Symbols
...
--		local stamp = pokemonData.stamps[slot]
		...
		pcall(function() button.QuantityLabel:Destroy() end)
...
--	local function updateList()
		...
		for i, stamp in pairs(inventory) do
			...
			button.MouseButton1Click:connect(function()
				...
				local n = #pStamps+1
				pStamps[n] = id
				pcall(function() ebuttons[n]:Destroy() end)
...
--	updateList()
	...
	pcall(function() sprite:destroy() end)
	closeButton:destroy()
	playButton:Destroy()
