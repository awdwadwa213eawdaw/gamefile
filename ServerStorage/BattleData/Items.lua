...
local function onTakeNecro(item, source)
	if item.ultraBurst == source.baseTemplate.baseSpecies then return false end
	return true
end
...
23:37:54.136 - No core Item data for bignugget
...
return {
	...
	['ultranecroziumz'] = {
		...
		onStart = function(pokemon)
			local template = self:getTemplate('Necrozma-Duskmane')
			local template2 = self:getTemplate('Necrozma-Dawnwings')         
			if pokemon.isActive and pokemon.baseTemplate == template or template2 then
				self:add('-activate', pokemon, 'item: Ultra Necrozium Z')
				local template = self:getTemplate('Necrozma-Ultra')
				...
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_FRONT/Necrozma-Ultra')
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_BACK/Necrozma-Ultra')
				pokemon.iconOverride = template.icon-1
				-- is there a better way to access this?
				pokemon.frontSpriteOverride = require(game:GetService('ServerStorage').Data.GifData)[shinyPrefix..'_FRONT']['Necrozma-Ultra']
				pokemon.baseStatOverride = template.baseStats
				self:add('detailschange', pokemon, pokemon.details,'[ultraN]', '[icon] '..(template.icon or 0))
	...
	['rustedsword'] = {
		...
		onSwitchIn = function(pokemon)
			if pokemon.isActive and pokemon.template.species == 'Zacian' then
				self:add('-activate', pokemon, 'item: Rusted Sword')
				local template = self:getTemplate('Zacian-Crowned')
				pokemon:formeChange(template)
				pokemon.baseTemplate = template
				pokemon.details = template.species .. ', L' .. pokemon.level .. (pokemon.gender == '' and '' or ', ') .. pokemon.gender .. (pokemon.set.shiny and ', shiny' or '')
				local shinyPrefix = pokemon.shiny and '_SHINY' or ''
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_FRONT/Zacian-Crowned')
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_BACK/Zacian-Crowned')
				pokemon.iconOverride = template.icon-1
				-- is there a better way to access this?
				pokemon.frontSpriteOverride = require(game:GetService('ServerStorage').Data.GifData)[shinyPrefix..'_FRONT']['Zacian-Crowned']
				pokemon.baseStatOverride = template.baseStats
				self:add('detailschange', pokemon, pokemon.details,'[rustedsword]', '[icon] '..(template.icon or 0))
		...
		onTakeItem = function(item, source)
			if source.template.species == 'Zacian' then return false end
			return true
		end
	},
	['rustedshield'] = {
		...
		onSwitchInPriority = -6,
		onSwitchIn = function(pokemon)
			if pokemon.isActive and pokemon.template.species == 'Zamazenta' then
				self:add('-activate', pokemon, 'item: Rusted Shield')
				local template = self:getTemplate('Zamazenta-Crowned')
				pokemon:formeChange(template)
				pokemon.baseTemplate = template
				pokemon.details = template.species .. ', L' .. pokemon.level .. (pokemon.gender == '' and '' or ', ') .. pokemon.gender .. (pokemon.set.shiny and ', shiny' or '')
				local shinyPrefix = pokemon.shiny and '_SHINY' or ''
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_FRONT/Zamazenta-Crowned')
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_BACK/Zamazenta-Crowned')
				pokemon.iconOverride = template.icon-1
				-- is there a better way to access this?
				pokemon.frontSpriteOverride = require(game:GetService('ServerStorage').Data.GifData)[shinyPrefix..'_FRONT']['Zamazenta-Crowned']
				pokemon.baseStatOverride = template.baseStats
				self:add('detailschange', pokemon, pokemon.details,'[rustedshield]', '[icon] '..(template.icon or 0))
				--	pokemon:setAbility(template.abilities[1])
				--	pokemon.baseAbility = pokemon.ability
				pokemon.typeOverride = template.types
			end
		...
		onTakeItem = function(item, source)
			if source.template.species == 'Zamazenta' then return false end
			return true
		end
	},
	['sawsbuckcoffee'] = {
	...
	['choiceband'] = {
		...
		onStart = function(pokemon)
			if pokemon.volatiles['choicelock'] then
				self:debug("Removing choicelock: " + pokemon.volatiles['choicelock'])
				pokemon:removeVolatile('choicelock')
			end
		end,
		onModifyAtk = function(atk, pokemon)
			if pokemon.volatiles ['dynamax'] then return end
			return self:chainModify(1.5)
		end,
	...
	['covertcloak'] = {
		...
		onModifySecondaries = function(secondaries)
			self:debug('Covert Cloak prevent secondary')
	...
	['ejectpack'] = {
		...
		onBoost = function(boost, target, source, effect)
			if self.activeMove.id == 'partingshot' then return end
			local eject = false
			...
			if eject then
				if target.hp then
					if not target.side:canSwitch(target.side) or target.forceSwitchFlag then return end
					if target.volatiles['commanding'] or target.volatiles['commanded'] then return end
					for i, pokemon in self:getAllActive() do
						if pokemon.switchFlag then return end
					end
					if target:useItem() then 
						target.switchFlag = true 
					end
	...
	['figyberry'] = {
		...
		onUpdate = function(pokemon)		
			if pokemon.hp <= pokemon.maxhp/2 then --it still fires at less than or half hp even for dynamax mons
				pokemon:eatItem()
			end
		end,
		onEatItem = function(item, pokemon)
	...
	['mewtwoniteshadowx'] = {
		id = "mewtwoniteshadowx",
		name = "Mewtwonite Shadow X",
		megaStone = "Mewtwo-MegaShadow-X",
		megaEvolves = "Mewtwo",
		onTakeItem = onTakeMegaStone,
	},
	['mewtwoniteshadowy'] = {
		id = "mewtwoniteshadowy",
		name = "Mewtwonite Shadow Y",
		megaStone = "Mewtwo-MegaShadow-Y",
		megaEvolves = "Mewtwo",
		onTakeItem = onTakeMegaStone,
	},
	['greninjite'] = {
	...
	['heavydutyboots'] = {
		...
		onUpdate = function(pokemon)
			if pokemon:hasType('Poison') then
				local sideConditions = 'toxicspikes'
				if pokemon.hp > 0 and pokemon.side:removeSideCondition(sideConditions) then
					self:add('-sideend', pokemon.side, self:getEffect(sideConditions).name, '[from] item = Heavy Duty Boots', '[of] ' .. pokemon)
				end
			end
		end,
	},
	...
	['safetygoggles'] = {
		...
		onImmunity = function(type, pokemon)
			if type == 'sandstorm' or type == 'snow' or type == 'powder' then return false end
		end,
	...
	['psychicseed'] = {
		...
		name = "Psychic Seed",
		onUpdate = function(pokemon)
			if self:isTerrain('psychicterrain') then
				pokemon:eatItem()
			end
		end,
		onEat = function(pokemon)
	...
	['mistyseed'] = {
		...
		name = "Misty Seed",
		onUpdate = function(pokemon)
			if self:isTerrain('mistyterrain') then
				pokemon:eatItem()
			end
		end,
		onEat = function(pokemon)
