...
return {
	brn = {
		...
		onResidual = function(pokemon)
			self:damage(pokemon.maxhp / 16)
		end,
	},
	par = {
		effectType = 'Status',
		onStart = function(target, sourcexw, sourceEffect)
	...
	dynamax = {
		...
		onStart = function(pokemon)
			...
			if pokemon.species.id == ('cramorantgulping' or 'cramorantgorging') then
				pokemon.formeChange('cramorant') --99% sure this is broken
			end
			if (pokemon.gigantamax) then
				storage.Models.Max.Value = 'Gmax'
				self:add('detailschange', pokemon, pokemon.details, '[forGmax]', '[icon] '..(pokemon.template.icon or 0))
				self:add('-formechange', pokemon, pokemon.template.species..'-Gmax')
				pokemon:formeChange(pokemon.template.species..'-Gmax')
				local shinyPrefix = pokemon.shiny and '_SHINY' or ''
				local spriteId = pokemon.template.baseSpecies..'-Gmax'
				...
				storage.Models.Max.Value = 'Dmax'
			end
			if (pokemon.template.species == 'Shedinja') then return end
			self:add('-start', pokemon, 'Dynamax')

			--Changes based on dynamax level, 2 is max (at LVL 10)
			local ratio = 2 --Gotta add Dynamax Levels
		...
		onTryAddVolatile = function(status, pokemon)
			if (status.id == 'flinch') then return null end
		end,
		...
		onBattleEnd = function(pokemon)
			if (pokemon.gigantamax) then self:add('-formechange', pokemon, pokemon.template.species)	
			end
			pokemon.currentDyna = false
			pokemon.hp = pokemon:getUndynamaxedHP() --Function in BattlePokemon
			pokemon.maxhp = pokemon.baseMaxhp
			self:add('-heal', pokemon, pokemon.getHealth, '[silent]') end,
		onEnd = function(pokemon)
			self:add('-end', pokemon, 'Dynamax')
			pokemon.currentDyna = false
			self:add('-formechange', pokemon, pokemon.template.species)
			pokemon:formeChange(pokemon.template.species)
			if (pokemon.template.species == 'Shedinja') then return end
			pokemon.hp = pokemon:getUndynamaxedHP() --Function in BattlePokemon
			pokemon.maxhp = pokemon.baseMaxhp
			self:add('-heal', pokemon, pokemon.getHealth, '[silent]')
	...
	maxraid = {
		...
		onStart = function(pokemon)
			...
			self:add('-start', pokemon, 'maxRaid', '[silent]') --Why did I never make it silent wtf
			if (pokemon.template.species == 'Shedinja') then --Allow Shedinja to appear in raids (todo nullify ability)
				pokemon.maxhp = 637
				pokemon.hp = 637
			else
				pokemon.maxhp = math.floor(pokemon.maxhp * 5)
				pokemon.hp = math.floor(pokemon.hp * 5)
			end
			self:add('-heal', pokemon, pokemon.getHealth, '[silent]')
		end,

		onTryAddVolatile = function(status, pokemon)
			if (status.id == 'flinch') then return end
		end,
		onSourceModifyDamage = function(damage, source, target, move) 
			if (move.id == 'behemothbash' or move.id == 'behemothblade' or move.id == 'dynamaxcannon') then
				return self:chainModify(2) --Does x2 dmg to dynamax pokemon
			end
		end,
		onBasePower = function(relayVar, source, target, move)
			if self.effectData.isEnraged and self.effectData.isEnraged > 0 then
				self.effectData.isEnraged -= 1
				if self.effectData.isEnraged == 0 then
					self:add("-raidenraged", source, 'ended')
				end
				return math.floor(move.basePower * (math.random(15, 20) / 20))
			end
		end,
		onDamagePriority = -40,
		onDamage = function(damage, target, source, effect)
			local reduced = false
			--print("Damage: " .. damage)
			if damage >= math.floor(target.maxhp/3) and Not(self.effectData.hasShield) then
				--print("Checked")
				damage = math.floor(target.maxhp/3)
				reduced = true
			end
			if self.effectData.hasShield and self.effectData.hasShield > 0 then
				self.effectData.hasShield -= 1
				damage = math.floor(damage * 0.3) --When shield up do 70% less damage
				reduced = true
			end
			if self.effectData.dmaxStages > 1 and (target.hp - damage <= math.floor(target.maxhp/3)) and not self.effectData.raidTrigger then
				--Do random effect when hit 1/3 marker of hp
				local r = math.random(1, 4)
				print(r)
				if r == 1 then --Heal effect
					if target.hp <= 0 or not target.hp or (target.hp - damage <= 0) then return end
					target.hp = math.floor(target.maxhp/math.random(4, 5))
					self:add("-heal", target, target.getHealth, "[silent]")
					target:cureStatus()
					self.effectData.raidTrigger = true
				elseif r == 2 then --Shield
					self.effectData.hasShield = 2
					self:add("-raidshield", target, self.effectData.hasShield) --Put up 2 stages of the shield (todo ui) also add to Queue
					self.effectData.raidTrigger = true
				elseif r == 3 then --Boost the pokemon by 2 stages in EVERY stat
					self:boost({atk= 2, spa= 2, spe= 2, def= 2, spd= 2}, target)
					self.effectData.raidTrigger = true
				elseif r == 4 then
					self.effectData.isEnraged = 5
					self:add("-raidenraged", target) --Attacks deal x1.5-2 the damage
					self.effectData.raidTrigger = true
					--IDK apply a random status to the target?
				end
			end
			if reduced then
			...
			if self.effectData.dmaxStages then
				self.effectData.dmaxStages -= 1
			end
		end,
		onDragOutPriority = 2,
		onDragOut = function(pokemon)
