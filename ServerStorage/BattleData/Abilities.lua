...
return {
	...
	['contrary'] = {
		onChangeBoost = function(boost, target, source, effect)
			if effect and effect.id == 'zpower' then return end
			for i, b in pairs(boost) do
				boost[i] = -b
			end
		end,
...
Add AlliesAndSelf()
	...
	['commander'] = {
		onUpdate = function(pokemon)
			...
			if (not ally or pokemon.baseTemplate.baseSpecies ~= 'Tatsugiri' or ally.baseTemplate.baseSpecies ~= 'Dondozo') then
			...
			if not pokemon:getVolatile('commanding') then
				...
				-- Add the volatiles
				self:add('-ability', pokemon, 'Commander')
