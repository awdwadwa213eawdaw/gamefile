...
return function(_p)--local _p = require(script.Parent)
	...
	local function onObtainItemSound()
		Utilities.sound(Audios.ObtainedItem, nil, nil, 10)
	end
	local function onObtainBadgeSound()
		Utilities.sound(Audios.ObtainedBadge, nil, nil, 10)
	end
	local function onObtainKeyItemSound()
		Utilities.sound(Audios.obtainedKeyItem, nil, nil, 10)
	end
	...
	_p.storage.Remote.PickUpManaphyEgg.PlaySound.OnClientEvent:connect(function(eggMain)
		...
		pcall(function()
			...
			Utilities.sound(10841831566, 4, nil, 10, eggMain)
		end)
	end)
	...
	_p.Network:bindEvent('appearShadowPortal', function(location)
		--todo scale anim
		print('APPEAR')
		pcall(function()
			if _p.DataManager.isDay then return end --double check (might be useless)
			if _p.DataManager.currentChunk.map:FindFirstChild('ShadowPortal') then return end
			-- Utilities.sound(7116090078, nil, nil, 10)

			local model = _p.storage.Models.ShadowPortal:Clone()
			model.Parent = _p.DataManager.currentChunk.map
			model:SetPrimaryPartCFrame(location)

			_p.ShadowVoid:doEvent(model)
		end)
	end)
	
	_p.Network:bindEvent('disappearShadowPortal', function()       
		--todo scale anim
		if not _p.DataManager.isDay then return end --double check (might be useless)
		pcall(function() _p.DataManager.currentChunk.map:FindFirstChild('ShadowPortal'):Destroy() end)
	end)
	...
	local doLeaveSafariDialogue = function()
		if _p.DataManager.currentChunk.id ~= 'chunk76' then return end
	...
	return {
		...
		onLoad_colosseum = function(chunk)
			local moveTutor = chunk.npcs.MoveTutor
			interact[moveTutor.model] = function() 
				...
				Utilities.fastSpawn(function()
					r = _p.Network:get('PDS', 'moveTutor')
					done = true
				end)
				...
				local moveTutor = function()
					...
					local payment = chat:choose('100 BP', '[$]100,000', 'Cancel')
					frame:Destroy()
					-- if cannot afford any

					if payment == 1 and r.bp < 100 then
						moveTutor:Say("It seems you don't have enough BP. Come back when you get more!")                           
						return
					elseif payment == 2 and r.money < 100000 then
						...
						if moveTutor:Say('[y/n]Would you like to buy some with ROBUX?') then
							...
							if _p.PlayerData.money < 100000 then -- we get the money from PD here because hopefully it got updated during the purchase process
					...
					if payment == 1 then
						moveTutor:Say("I'll take that 100 BP now.")
					elseif payment == 2 then
						moveTutor:Say("I'll take that [$]100,000 now.")
					end
			...
			local bottlecap = chunk.npcs.bottlecap
				...
				if chat:say(bottlecap, '[y/n]In exchange for Bottle Caps, that is! Wanna try?') then
					...
					if useCapType == 'Silver Bottle Cap' then
						...
						if chat:say(bottlecap, '[y/n]Alright, so you want to Hyper Train in '..choice..'?') then
							...
							chat:say(bottlecap, 'We\'ve successfully trained your '..choice..' stat!')
							chat:say(bottlecap, 'I\'ll take that Silver Bottle Cap now.')
							chat:say(bottlecap, 'Thanks! Never stop training!')
							spawn(function() _p.Menu:enable() end)
						else
							chat:say(bottlecap, 'Okay, come back if you DO want to train your pokemon.')
							spawn(function() _p.Menu:enable() end)
						end
					else
						local trained = _p.Network:get('PDS', 'getivs', slot, 'all')
						if chat:say(bottlecap, '[y/n]Alright, so you want to Hyper Train all of your pokemon\'s stats?') then
							if trained then
								chat:say(bottlecap, 'The Pokemon\'s stats are already maxed.')
								spawn(function() _p.Menu:enable() end)
								return
							end
							chat:say(bottlecap, 'Alright! Time to do some Hyper Training!')
							...
							chat:say(bottlecap, 'Success!')
							chat:say(bottlecap, 'We\'ve successfully trained all of your pokemon\'s stats!')
		...
		onLoad_resort = function(chunk)
			...
			local look = workspace.WorldModel.Building.H4CK3DPC.look
			local easteregg = chunk.npcs.ee
			
			interact[easteregg.model] = function()
				interact[easteregg.model] = nil
				MasterControl:Stop()
				MasterControl.WalkEnabled = false
				easteregg:LookAt(root.Position)
				easteregg:Say('...')
				easteregg:LookAt(look.Position)
				MasterControl.WalkEnabled = true
			end
			
			local names = {"WafflemanXx", "johnlitt"}
			local anims = {1, 2, 3}]]
			
			--[[for i, name in names do
				local npc = chunk.npcs[name]
				
				if npc then
					local animId = math.random(#anims)
					
					npc.humanoid:LoadAnimation(create('Animation')({ ]]
						--AnimationId = 'rbxassetid://'.._p.animationId["NPCDance"..anims[animId]] ]]
					--})):Play()
					
					--table.remove(anims, animId)
				--end
			--end
			
			--OLD RESORT HATCH: UNCOMMENT IF REVERTING TO IT!
			pcall(function()
			...
			if completedEvents.MeetJake then
				...
				local function pickStarter(og)
					...
					local chooser; chooser = create 'ImageButton' { -- 600x200
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://5227165907', -- StartersBG
					...
					create 'ImageButton' {
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://6604520048', -- Arrow Right Outline
						Rotation = 180,
						SizeConstraint = Enum.SizeConstraint.RelativeYY,
				...
				local function setupDevProduct()
					...
					interact[david.model] = function()
						spawn(function() _p.Menu:disable() end)
						local res = (function()
							if not chat:say(david, 'The professor sells these pokemon for 34 R$ each.', '[y/n]Would you like to purchase one?') then return end
				...
				local function setupJakeFight(jake, prof, mom, dad)
					...
					_p.DataManager:preload(16697562990)
					touchEvent('JakeBattle', room.model.CutsceneTrigger2, false, function()
						MasterControl.WalkEnabled = false
						spawn(function() _p.Menu:disable() end)
					...
					if completedEvents.MeetParents then
						...
						touchEvent('ChooseFirstPokemon', room.model.CutsceneTrigger, false, function()
							...
							chat:say(prof, 'Yes, and there are still many things that we do not know about pokemon.',
								...
								'Make a choice from these 27 different breeds.',
								'Go ahead now.')
						...
						interact[room.model.David] = {'The professor sells these pokemon for 34 R$ each.',
							'I\'m pretty sure he plans on giving you your first one free, though.'}
					end
				elseif not completedEvents.JakeBattle then
		...
		onDoorFocused_Gate1 = function()
			...
			local music = Utilities.loopSound(16697552590)
		...
		onLoad_chunk2 = function(chunk)
			local function setupForestBattle()
				_p.DataManager:preload(6860109291, 10841931313)
				...
				interact[linda.model] = function()
					...
					local win = _p.Battle:doTrainerBattle {
						IconId = 6860109291,
						musicId = Audios.EclipseGruntBattle1,
						PreventMoveAfter = true,
		...
		onBeforeEnter_PokeCenter = function(room)
			...
			if npc then
				...
				interact[npc.model] = function()
					local pokeballs
					Utilities.fastSpawn(function()
						local ballNums = _p.Network:get('PDS', 'getHealing', true)
						local ballNames = {}
					...
					if not healed then
						...
						spawn(function() 
							_p.Network:get('PDS', 'getHealing')
							if not _p.Network:get('PDS', 'getHealing') then
								chat:say(npc, 'Sorry, there was an issue with processing your team, please try again.') 
							else
		...
		onBlackoutFailsafe = function()
			local regionData = _p.DataManager.currentChunk.regionData
			_p.MusicManager:popMusic('RegionMusic', 0.1)
			_p.MusicManager:stackMusic(regionData.Music, 'RegionMusic', 1)
		end,
		...
		onBeforeEnter_EeveeHouse = function(room)
			local inGroup = _p.player:IsInGroup(35110780)
		...
		onLoad_chunk7 = function(chunk)
			...
			if completedEvents.GroudonScene then
				...
				_p.DataManager:preload(10841931313, 6860109291, 10841856155, 10841858409,10841860027, 68068592, 317480860) -- admin music, eclipse logo, earthquake, groudon scene music[2], particle, pulse
				...
				touchEvent('GroudonScene', chunk.map.GroudonTrigger, false, function()
					...
					local win = _p.Battle:doTrainerBattle {
						IconId = 6860109291,
						musicId = Audios.EcliseGruntBattle1,
						PreventMoveAfter = true,
						LeaveCameraScriptable = true,
					...
					local sceneMusic = Utilities.sound(10841858409, .3)
					delay(99, function()
					...
					delay(.3, function()
						local p = create 'Part' {
							...
							create 'Decal' {
								Texture = 'rbxassetid://13045721033', -- Circle Outline W
					...
					if want then
						...
						elseif state == 'full' then
							dcm:Say('Oops, looks like you\'ve got a full team already.',
								...
								'Why don\'t you deposit one of your pokemon in the PC to make space?')
							return
						end
						wait()
						chat.bottom = true
				...
				elseif option == 2 then
					...
					if poke then
						...
						if take then
							...
							local teach = false
							if poke.lmove then
								teach = dcl:Say("Oh, a quick question.", "[y/n]If your " .. poke.name .. " had tried to learn a new move, would you have wanted me to allow it?")
								dcl:Say("Oh, good. That's exactly what I did.")
							end

							local done = false
							Utilities.fastSpawn(function()
								_p.Network:get('PDS', 'takeDCPokemon', pChoice, teach)
								done = true
							end)
...
--[[					if #_p.PlayerData.party == 6 and not _p.PlayerData.pc:hasSpace() then  -- todo?
	...
	onLoad_chunk10 = function(chunk)
			...
			interact[claus.model] = function() --'We grow trees here and sell them around Christmas each year.'
		...
		onBeforeEnter_Kresmas = function(room)
			...
			interact[hcman.model] = {'During the holidays, we enjoy selling winter themed items here.', --(Non Winter)
				'I really do enjoy the holiday season...'}
			--[[ interact[hcman.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(hcman, 'Welcome to my personal Winter Shop!', 'I sell tons of winter-related items that are only available during the holiday season.', 'So how may I help you?')
				_p.Menu.wntrshop:open('winter')
				chat:say(hcman, 'Thank you, please come again!')
				_p.Menu:enable()
			end	
		...
		--	spawn(function() _p.PlayerData:completeEvent('GivenSnover') end)
		--end,
			...
			if completedEvents.LighthouseScene then
				...
				connectLighthouseScene = function()
					...
					touchEvent('LighthouseScene', lhtrigger, false, function()
						...
						_p.DataManager:preload(337448917, 243728104, 10841873457, 10841873457, 337973384, 317480860, 338262406)
						_p.DataManager:preloadModule('AnchoredRig')
						...
						local win = _p.Battle:doTrainerBattle {
							IconId = 6860109291,
							musicId = {Audios.EcliseGruntBattle1, Audios.EcliseGruntBattle2},
							musicVolume = 2,
							PreventMoveAfter = true,
							LeaveCameraScriptable = true,
						...
						local win = _p.Battle:doTrainerBattle {
							IconId = 6860109291,
							musicId = Audios.EclipseAdminFight,
							PreventMoveAfter = true,
							LeaveCameraScriptable = true,
							trainerModel = admin.model,
							num = 71
						...
						local lighting = game:GetService('Lighting')
						--// CHRISTMAS EVENT
						--local snowfall = chunk.map.Snowmaker:GetChildren()
						--for _,v in pairs(snowfall) do
						--	v.Enabled = false
						--end
						...
						delay(.3, function()
							local p = create 'Part' {
								...
								create 'Decal' {
									Texture = 'rbxassetid://13045721033',
									Face = Enum.NormalId.Top,
								}
							}
							for i = 1, 3 do
						...
						local particles = create 'ParticleEmitter' {
							Texture = 'rbxassetid://13045723591', -- Hearts
						...
						jake.model.Head.face.Texture = 'rbxassetid://13045827158' -- Joyful Smile
						delay(1, function()
							particles:Destroy()
						end)
						MasterControl:LookAt(jp)
		...
		onBeforeEnter_Gym3 = function(room, continueCFrame)
			...
			interact[leader.model] = function()
				...
				local win = _p.Battle:doTrainerBattle {
					...
					musicId = Audios.Gym3LeaderFight,
					PreventMoveAfter = true,
			...
			for _, m in pairs(room.model.Candles:GetChildren()) do
				if m:FindFirstChild('Wick') then
					if math.random(30) == 23 then
						...
						pe.Texture = 'rbxassetid://13045832392'  -- Pink Orb
						...
						chat.silentInteract[m] = function()
							...
							delay(3, function()
								pe.Texture = 'rbxassetid://13045835106' -- Red Orb
								pe.Color = ColorSequence.new(
									Color3.new(1, .6, .04),
									Color3.new(1, .92, .06))
							end)
			...
			if _p.PlayerData.rotomEventLevel == 3 then
				...
				spawn(function()
					...
					local decal = create 'Decal' {
						Texture = 'rbxassetid://13045838457', -- TV Static
						Face = Enum.NormalId.Front,
						Parent = screen,
					}
					while tag.Parent do
		...
		onBeforeEnter_HMBadBedroom = function(room)
			if _p.PlayerData.rotomEventLevel == 6 then
				...
				local decal = create 'Decal' {
					Texture = 'rbxassetid://13045838457',
					Face = Enum.NormalId.Top,
		...
		onHauntedGameBoyClicked = function(gameboy)
			...
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Gameboy,{battleSceneType = 'HauntedMansion', musicId = {Audios.hauntedm1, Audios.hauntedm2}})
		end,
		...
		onLoad_chunk14 = function(chunk)
			...
			if not completedEvents.Jirachi then
				_p.DataManager:preload(10842008688, 10840573719)
				...
				chatCn = _p.player.Chatted:connect(function(msg)
					...
					if MasterControl.WalkEnabled and not _p.DataManager.isDay and msg:lower():match('^i%s+wish%s+%a') then
						...
						if pos.Y > 0 and pos.Y < 5 and math.sqrt(pos.X^2+pos.Z^2) < 3.5 then
							...
							jirachi.Parent = map
							_p.MusicManager:stackMusic(10842008688, 'Jirachi')
						...
						--]]
							...
							_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Wish,{battleSceneType = 'Jirachi', musicId = Audios.LegendsBattle})
...
--				blimpEngine.Anchored = true
		...
		onBeforeEnter_LottoShop = function(room)
			...
			interact[hobo.model] = function()
				...
				elseif triesToday > 0 then
					if not hobo:Say("So far, you've drawn " .. triesToday .. " ticket" .. (triesToday == 1 and "" or "s") .. " today.", "You can draw " .. 4 - triesToday .. " more ticket" .. (triesToday == 3 and "" or "s") .. " for 29 R$" .. (triesToday == 3 and "." or " each."), "[y/n]Would you like to purchase a ticket for 30 R$?") then
						hobo:Say("Please do visit again!")
						return
					end
				end
				...
				local function draw()
					...
					if triesToday < 4 then
						chat:say(head, "Today, you can draw up to " .. 4 - triesToday .. " more ticket" .. (triesToday == 3 and "" or "s") .. " for 30 R$" .. (triesToday == 3 and "." or " each."))
					end
		...
		onLoad_chunk20 = function(chunk) -- ANTHIAN: SHOPPIN
			...
			interact[dealer.model] = function()
				local buy = dealer:Say('Hello there, young trainer.',
					...
					'I\'ll turn him over to you for a mere 399 ROBUX.',
				...
				spawn(function() dealer:Say('[ma]Alright, let me see your ROBUX...') end)
				local loadTag = {}
				_p.DataManager:setLoading(loadTag, true)
		...
		onBeforeEnter_HerosHoverboards = function(room)
			...
			local function onClickHoverboard(model)
				...
				if model.Name:sub(1, 6) == 'Basic ' then
					...
					if _p.Network:get('PDS', 'ownsHoverboard', model.Name) then
						...
						if shopGuy:Say('[y/n]Ah, '..model.Name..'... Would you like to purchase this one for 14 R$?')
			...
			interact[salesperson.model] = function()
				...
				elseif choice == 2 then
					...
					if room.model.PaidBoards:FindFirstChild("White Fidget Spinner") then
						Utilities.lookAt(room.model.DeccaCF.CFrame)
					else
						Utilities.lookAt(CFrame.new(22.6, 10, 10.9, -0.994, 0.018, -0.111, 0, 0.988, 0.157, 0.112, 0.156, -0.981) + room.basePosition)
					end
		...
		onBeforeEnter_Gym4 = function(room)
			...
			if _p.PlayerData.badges[4] then
				...
				local function rollOut()
					...
					local win = _p.Battle:doTrainerBattle {
						musicId = Audios.Gym4LeaderFight,
						PreventMoveAfter = true,
						trainerModel = leader.model,
		...
		onLoad_chunk22 = function(chunk, data)
			...
			if _p.PlayerData.badges[4] and not completedEvents.DefeatTEinAC then
				_p.DataManager:queueSpritesToCache({'_FRONT', 'Absol'}) -- for the cry
				_p.DataManager:preload(280857070, 10841914611, 507289472, 334858056, 509072758, 509073816, 10840573719)
				...
				spawn(function()
					...
					schedule(changeAnim, 'Grab', .3, function()
						...
						attach(pm.LL, dummy['Left Leg']) -- Eclipse Shirts [BELOW]
						pm.Shirt.ShirtTemplate = 'rbxassetid://13045972661' -- Shirt B
						pm.Pants.PantsTemplate = 'rbxassetid://13045843076' -- Shirt A
					...
					local st = tick()
					Utilities.sound(10841914611, nil, nil, 15)
					...
					-- Dialga and Palkia
					_p.MusicManager:stackMusic(11205007994, 'Cutscene')
					--				wait(.75)
			...
			--// Revamped Winter Event 2022
			--[[ local lawyer = chunk.npcs.LawyerDude
		...
		onLoad_chunk24 = function(chunk)
			...
			local function updateFogColor()
				...
				if _p.ShadowVoid.inVoid then return end
		...
		onBeforeEnter_RVHouse = function(room)
			local revealglass = room.npcs.forcesguy
			interact[revealglass.model] = function()
				chat:say(revealglass, 'Legend has it that there were three dangerous beasts harming the area just outside of Aredia.',
					'I study more and more about it every day to figure out the catastrophic damages dealt on Route 12 and beyond.')
				if completedEvents.GetRevealGlass then return end
				if _p.Network:get('PDS', 'has3forces') then
					Utilities.exclaim(revealglass.model.Head)
					chat:say(revealglass, 'Those... those are the Forces Of Nature!',
						'...',
						'I want you to have something I found a very long time ago.',
						'It is said to summon a true power that only these Pokemon can control.',
						'There is nobody more fit to carry this legendary artifact than you.',
						'Please, take it.')
					chat.bottom = true
					onObtainKeyItemSound()
					...
					chat.bottom = false
					chat:say(revealglass, 'I don\'t know the exact nature of the relationship between the Reveal Glass and the Forces Of Nature, but in my eyes, I am sure you are the one destined to find the truth.')
				end
			end
		end,
		...
		onLoad_gym5 = function(chunk) -- gym 5 is a chunk
			...
			interact[leader.model] = function()
				if _p.PlayerData.badges[5] then
					...
					local win = _p.Battle:doTrainerBattle {
						musicId = Audios.Gym2LeaderFight,
						PreventMoveAfter = true,
						trainerModel = leader.model,
		...
		onLoad_chunk34 = function(chunk)
			...
			if map:FindFirstChild('Victini') then
				_p.DataManager:preload(10063618368)
				_p.DataManager:preloadModule('AnchoredRig')
			else
		...
		onLoad_chunk43 = function(chunk)
			if completedEvents.TERt14 then
				...
				_p.DataManager:preload(10842092577)
				...
				touchEvent(nil, map.CTrigger, false, function()
					...
					_p.MusicManager:stackMusic(10842092577, 'Cutscene')
					wait(.5)
					local speed = .5
					local function schedule(animTrack, kfName, kfTime, func)
						local fired = false
		...
		onLoad_chunk44 = function(chunk)
			--		suicune,10006321060,115.93,1.06
		...
		onLoad_chunk45 = function(chunk)
			--// Winter Event 2022
			--[[ local guy = chunk.npcs.megastoneguy
			local ChristmasTree = chunk.map.Tree
			if completedEvents.CollectMiniors then
				for i,v in pairs(ChristmasTree.Ball:GetChildren()) do
					v.Transparency = 0
				end
			end

			if completedEvents.CollectComfeys then
				for i,v in pairs(ChristmasTree.Light:GetChildren()) do
					v.Transparency = 0
				end
			end

			if completedEvents.CollectStarItems then
				ChristmasTree.Star.Transparency = 0
			end
			...
			interact[guy.model] = function()
				...
				if not completedEvents.LearnAboutSceptile then
				...
				spawn(function() _p.PlayerData:completeEvent('LearnAboutSceptile') end)
				if not completedEvents.CollectMiniors then
					guy:Say(
				...
				local stage1 = _p.Network:get('PDS', 'getCSceptileStage', 1)
				if stage1 then
					guy:Say(
						'...',
				...
				if not completedEvents.CollectComfeys and completedEvents.CollectMiniors then
					guy:Say(
				...
				local stage2 = _p.Network:get('PDS', 'getCSceptileStage', 2)
				if stage2 then
					guy:Say(
						'...',
						'Absolutely amazing!', 
				...
				if not completedEvents.CollectStarItems and completedEvents.CollectComfeys then
					guy:Say(
				...
				local stage3 = _p.Network:get('PDS', 'getCSceptileStage', 3)
				if stage3 then
					guy:Say(
						'...',
					...
					chat.bottom = true
					onObtainItemSound()
					chat:say('Obtained a Sceptilite C!', _p.PlayerData.trainerName .. ' put the Sceptilite C in the Bag.')
					chat.bottom = nil
				end 
			end --]]
			...
			local p1 = p0 + Vector3.new(-116, -20, -15)
			local p2 = p1 + Vector3.new(-68, -12, -114)
		...
		onLoad_gym7 = function(chunk)
			_p.DataManager:loadModule('Gym7'):activate(chunk)
		...
		onUnload_gym7 = function()
			local m = _p.DataManager:getModule('Gym7')
			if m then
				m:deactivate()
			end
		end,
		...
		onLoad_chunk51 = function(chunk)
			...
			touchEvent('Regigigas', chunk.map.Trigger, true, function()
				...
				MasterControl:Stop()
				workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
				workspace.CurrentCamera.CFrame = chunk.map.CAM.CFrame
				wait(5)
		...
		onLoad_chunk58 = function(chunk)
			...
			interact[captain.model] = function()
				...
				if not completedEvents.TalkToCap or not has then
					...
					spawn(function() _p.PlayerData:completeEvent('TalkToCap') end)
				else
					...
					if #options > 0 then
						...
						if choice == 'Frigidia Island' then
							...
							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(480.595, 17.733, 747.385))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
						...
						if choice == 'Voltridia Island' then
							...
							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(2248.195, 1, 923.29))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
						...
						if choice == 'Obsidia Island' then
							...
							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(1187.177, 24.642, 890.488))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
		...
		onLoad_chunk61 = function(chunk)
			...
			if not completedEvents.MeetTessBeach then
				...
				touchEvent("MeetTessBeach", chunk.map.MeetTessTrigger, true, function()
					...
					fisherman:LookAt(CFrame.new(-826.527, 5.624, 7732.488))
					...
					spawn(function()
						Utilities.lookAt(CFrame.new(-812.882507, 11.1687737, 7740.04395, 0.45704028, -0.147810057, 0.877078354, -7.45057971E-09, 0.986095071, 0.16618216, -0.889445961, -0.0759519488, 0.450685203))
					end)
					spawn(function()
					...
					Jake:Teleport(CFrame.new(-731.514, 27.443, 7765.493))
					Jake:LookAt(CFrame.new(-830.849, 5.377, 7739.622))
					local CameraCFrame = CurrentCamera.CFrame
					local u601 = CFrame.new(-727.291504, 29.887804, 7768.62793, 0.269544631, -0.182833284, 0.945472181, 3.72529074E-09, 0.981811166, 0.189860404, -0.962987959, -0.0511758476, 0.264641881)
					Utilities.Tween(8.5, "linear", function(p553)
						CurrentCamera.CFrame = CameraCFrame:Lerp(u601, p553)
					end)
					wait(3)
					Jake:WalkTo(Vector3.new(-731.515, 27.442, 7798.938))
					Jake:Destroy()
					Utilities.lookBackAtMe()
					MasterControl.WalkEnabled = true
					_p.RunningShoes:enable()
		...
		onLoad_chunk63 = function(chunk)
			...
			if Cobalion and Terrakion and Virizion then
				touchEvent('SwordsOJ', chunk.map.SwordsOJTrigger, true, function()
					...
					Utilities.lookAt(CFrame.new(117.8, 86.701, -448.4, 0.939692557, -0.0885213092, 0.330366284, 0, 0.965925872, 0.258818835, -0.342020333, -0.243210152, 0.907673359))
					wait(1)
		...
		onLoad_chunk64 = function(chunk)
			local map = chunk.map
			local Lapras = map:FindFirstChild("Lapras")
		...
		onLoad_chunk70 = function(chunk)
			if completedEvents.Articuno then
				chunk.map.Articuno:Destroy()
			else
				local Articuno = chunk.map:FindFirstChild('Articuno')
				local main = Articuno.Main
				local mcf = main.CFrame
				local parts = {}
				local inv = mcf:inverse()
				for _, p in pairs(Utilities.GetDescendants(Articuno, 'MeshPart')) do
					if p ~= main then
						parts[p] = inv * p.CFrame
					end
				end
				spawn(function()
					local st = tick()
					while chunk and Articuno do
						local et = (tick()-st)*3
						local cf = mcf + Vector3.new(0, math.sin(et)*2, 0)
						main.CFrame = cf
						for p, rcf in pairs(parts) do
							p.CFrame = cf * rcf
						end
				...
				Articuno.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.ArticunoIdle }):Play()
			end
		end,
		onLoad_chunk71 = function(chunk)
			if completedEvents.Zapdos then
				chunk.map.Zapdos:Destroy()
			else
				local Zapdos = chunk.map:FindFirstChild('Zapdos')
				local main = Zapdos.Main
				local mcf = main.CFrame
				local parts = {}
				local inv = mcf:inverse()
				for _, p in pairs(Utilities.GetDescendants(Zapdos, 'MeshPart')) do
					if p ~= main then
						parts[p] = inv * p.CFrame
					end
				end
				spawn(function()
					local st = tick()
					while chunk and Zapdos do
						local et = (tick()-st)*4
						local cf = mcf + Vector3.new(0, math.sin(et)*3, 0)
						main.CFrame = cf
						for p, rcf in pairs(parts) do
							p.CFrame = cf * rcf
						end
				...
				Zapdos.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.ZapdosIdle }):Play()
			end
		end,
		onLoad_chunk72 = function(chunk)
			if completedEvents.Moltres then
				chunk.map.Moltres:Destroy()
			else
				local Moltres = chunk.map:FindFirstChild('Moltres')
				local main = Moltres.Main
				local mcf = main.CFrame
				local parts = {}
				local inv = mcf:inverse()
				for _, p in pairs(Utilities.GetDescendants(Moltres, 'MeshPart')) do
					if p ~= main then
						parts[p] = inv * p.CFrame
					end
				end
				...
				Moltres.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.MoltresIdle }):Play()
			end
		end,
		onLoad_chunk73 = function(chunk)
			local map = chunk.map
			local camTween =CFrame.new(-2389.319, 519.674, -4879.626) * CFrame.Angles(0,0,0)
			local camTween1 =CFrame.new(-2433.436, 512.244, -4869.635) * CFrame.Angles(math.rad(-40),0,0)
			local camTween2 =CFrame.new(-2373.805, 521.773, -5012.595) * CFrame.Angles(math.rad(-10),math.rad(150),0)
			local camTween3 =CFrame.new(-2370.554, 500.34, -4967.903) * CFrame.Angles(math.rad(10),math.rad(150),0)
			if completedEvents.GetSWing and not completedEvents.Lugia then
				touchEvent('Lugia', map.Trigger, true, function()
					...
					MasterControl.WalkEnabled = false
					MasterControl:Stop()
					MasterControl:WalkTo(Vector3.new(-2381, 488.601, -4950))
					MasterControl:LookAt(CFrame.new(-2437.057, 496.518, -4950.831))
					cam.CFrame = CFrame.new(Vector3.new(-2353.305, 503.164, -4949.375))
					wait(1)
...
--[[		onLoad_chunk74 = function(chunk)
			spawn(function()
				wait(5)
				if not game.Workspace:FindFirstChild("MarshadowTime") then
					print("Failsafe Initiated")
					local oldChunk = _p.DataManager.currentChunk
					Utilities.TeleportToSpawnBox()
					oldChunk:Destroy()
					chunk = _p.DataManager:loadChunk('chunk9')
					wait(3)
					spawn(function()
		...
		onLoad_chunk75 = function(chunk)
			...
			-- BGC Here, can't use touchEvent because it disconnects the fn after one touch :(
		...
		["onDoorFocused_C_chunk75|a"] = doLeaveSafariDialogue,
		["onDoorFocused_C_chunk75|b"] = doLeaveSafariDialogue,
		leaveSafari = function(chunk, isForced)
			...
			Utilities.TeleportToSpawnBox()
			chunk:destroy()
			chunk = _p.DataManager:loadChunk('chunk75')
		...
		onLoad_chunk77 = function(chunk)
			...
			local positions = {
				pos1 = CFrame.new(25.383, 22.91727, 5187.016), 
				pos2 = CFrame.new(5.021, 22.91727, 5127.564), 
				pos3 = CFrame.new(-77.995, 22.91727, 5115.42), 
				pos4 = CFrame.new(-444.363, 22.91727, 4815.131), 
				pos5 = CFrame.new(-448.539, 22.91727, 4730.805), 
				pos6 = CFrame.new(-550.234, 22.91727, 4462.768), 
				pos7 = CFrame.new(-605.84, 22.91727, 4455.687), 
				pos8 = CFrame.new(-569.302, 22.91727, 4437.501), 
				pos9 = CFrame.new(-185.327, 22.91727, 4659.189), 
				pos10 = CFrame.new(-168.164, 22.91727, 4714.135), 
				pos11 = CFrame.new(-156.204, 22.91727, 4670.055), 
				pos12 = CFrame.new(482.798, 22.91727, 5367.738), 
				pos13 = CFrame.new(441.157, 22.91727, 5402.645), 
				pos14 = CFrame.new(502.376, 22.91727, 5405.072)
			}
			local function areachecker()
				if _p.DataManager.currentChunk.id ~= "chunk77" then
					return
				end
		...
		onLoad_chunk79 = function(chunk)
			...
			if not completedEvents.EclipseBaseReveal then
				touchEvent('EclipseBaseReveal', chunk.map.cTrigger2, true, function()
					...
					spawn(function() cutgrunt:WalkTo(Vector3.new(-1719.875, 29.253, -368.026)) end)
					wait(.5)
					tdoor:open(.3)
					wait(1)
					tdoor:close(.3)
		...
		onBeforeEnter_Tavern = function(room)
			touchEvent(nil, room.model.Elevator.Trigger, true, function()
				...
				wait()
				local newChunk = _p.DataManager:loadChunk('chunk80')
		...
		onLoad_chunk80 = function(chunk)
			touchEvent(nil, chunk.map.ElevatorTrigger, false, function()
				...
				wait(0.5)
				wait()
				local newChunk = _p.DataManager:loadChunk("chunk79")
		...
		onLoad_chunk86 = function(chunk)
			...
			touchEvent('ParentalSightings', chunk.map.TRIGGER, true, function()
				local door = chunk:getDoor('C_chunk87')
				MasterControl.WalkEnabled = false
				MasterControl:Stop()
				spawn(function()
					_p.Menu:disable()
		...
		onExitC_chunk86 = function()
			local chunk = _p.DataManager.currentChunk
			if chunk.id ~= 'chunk87' then return end
			if not completedEvents.DefeatEclipseBase then
				...
				spawn(function()
					wait(2)
					_p.MusicManager:stackMusic(11307978581, 'Cutscene')
				end)
		...
		onLoad_chunk90 = function(chunk)
			...
			if not events.DefeatHoopa then
				...
				touchEvent(nil, chunk.map.HTrigger, false, function()
					...
					Professor:SayFor(1, "Harry, the bottle.") 
					EclipseAdmin:SayFor(1, "Yes, sir.") 
					...
					Professor:SayFor(3, "I just love it when everything goes according to plan!")
					spawn(function()
					...
					spawn(function()
						Jake:LookAt(Vector3.new(-290.286, 1768.384, 7.302))
						Dad:SayFor(3, "You're making a huge mistake.", 2, "You don't know what kind of power you're messing with.")
						Professor:SayFor(2, "Ah, that's where you're wrong.", 2, "I have a pretty good idea of the power I'm messing with.")
						Professor:LookAt(pos1)
						chat.bottom = true
						...
						Professor:SayFor(3, "It's the power to travel to other worlds.", 3, "The power to become the creator of my own reality.", 3.6, "The power to control right and wrong.")
						chat.bottom = nil
						...
						Professor:LookAt(Vector3.new(-301.732, 1765.59, 6.84))
						Professor:SayFor(2, "".. _p.PlayerData.trainerName .. ", I have no further need of you.", 2.5, "I suspect I have to defeat you in battle to get rid of you for good.", 2, "I'll try not to have too much fun with this.")
						delay(2, function()
						...
						if win then
							...
							Professor:SayFor(1, "Enough nonsense!")
							Professor:LookAt(pos1)
							spawn(function()
							...
							Professor:SayFor(1, "Hoopa!", 2, "It is I who has awoken you this day!", 2, "Grant me my wish, that I may discover a pure world in need of ruling!")
							wait(0.5)
							spawn(function()
							...
							local portalscreenn = _p.AnimatedSprite:new({
								sheets = { {
									id = 509072758, 
									rows = 4
								}, {
							...
							portalscreenn.spriteLabel.Parent = newChunk.map.PortalGui.SurfaceGui
							portalscreenn.spriteLabel.ImageTransparency = 1
							portalscreenn:Play()
							spawn(function()
								Tween(2, nil, function(p755)
									portalscreenn.spriteLabel.ImageTransparency = 1 - 1 * p755
								end)
							end)
							spawn(function()
							...
							wait(1)
							Professor:SayFor(3, "Isn't it glorious?", 2.5, "You will all soon see that I'm not as mad as you believe.", 3, "In fact, I am the most clever of any of you.")
							Professor:SayFor(2.5, "I am only soul wise enough to devise such a plan, and brave enough to carry it out.", 2.5, "The almighty Hoopa, a Pokemon so talented and powerful...", 3, "Ancient Rorians were so full of fear, they decided to imprison it here!")
							spawn(function()
								local proot = Professor.model.HumanoidRootPart
								create 'Weld' {
									Part0 = Professor.model.Head,
									Part1 = Professor.model.Hair,
									C1 = Professor.model.Hair.CFrame:inverse() * Professor.model.Head.CFrame,
									Parent = Professor.model.Head,
								}
								create 'Weld' {
									Part0 = Professor.model.Head,
									Part1 = Professor.model.Glasses,
									C1 = Professor.model.Glasses.CFrame:inverse() * Professor.model.Head.CFrame,
									Parent = Professor.model.Head,
								}
								Professor.model.Humanoid:Destroy()
								Professor.humanoid = create 'Humanoid' {DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None, Parent = Professor.model}
								for _, ch in pairs(Professor.model:GetChildren()) do if ch:IsA('BasePart') then ch.Anchored = false end end
								Professor.position = create 'BodyPosition' {MaxForce = Vector3.new(2e4, 0, 2e4), Position = proot.Position, Parent = proot}
								Professor.gyro = create 'BodyGyro' {MaxTorque = Vector3.new(math.huge, 9e3, math.huge), CFrame = proot.CFrame, P = 1e4, Parent = proot}
								Professor.animated = true
							end)
							spawn(function()
								...
								wait(0.3)
								Professor.humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.cypressToss }):Play()
								wait(1.6)
								Professor:destroy()
							end)
							spawn(function()
							...
							Professor:SayFor(1, "And here it stayed, waiting for me to come along one day and-")
							chat:SayFor(1, "WHAT ARE YOU DOING?!")
							wait(1)
							...
							Professor:SayFor(3, "Am I the only one who's tired of that guys monologuing?", 2, "I've been holding that in for a long time...")
							spawn(function()
								EclipseAdmin:LookAt(Vector3.new(-290.273, 1768.1, 4.955))
								spawn(function()
							...
							LHCAdmin:SayFor(2, "Jake, what is the meaning of this?")
							Jake:SayFor(2, "It's the end of the line for Team Eclipse.", 2, "Nobody comes between me and my friends.")
							Jake:SayFor(2, "Cypress made a huge mistake thinking he could take advantage of us like that.")
							LHCAdmin:SayFor(2, "You're making a huge mistake if you think you've stopped Team Eclipse.", 3, "Cypress is on his way to the new world as we speak!", 2, "This is only the beginning!")
							Jake:SayFor(3, "You'd better join him, or you're next!")
							LHCAdmin:SayFor(1.5, "Say no more!")
							...
							Professor:SayFor(2, "This portal will continue to grow out of control until Hoopa is defeated.", 1.5, "I will stay here and fight.", 2, "The rest of you need to leave now.")
							spawn(function()
								Jake:LookAt(Vector3.new(-289.29, 1768.14, -7.256))
							end)
							Tess:SayFor(2, "Jake, we have fought so long to rescue you!", 2, "I am not leaving your side now.")
							Jake:SayFor(1.5, "The fight is not yet over.", 2, "I need you to get to safety so I can finish this.", 2, "The ancient Rorians bound Hoopa because it destroyed their homes and way of life.")
							Jake:SayFor(1.5, "If we aren't careful, the same will happen to us.", 2, "Cypress planned to leave this world destroyed by Hoopa.", 2, "Hoopa must be stopped before it's too late.")
							Jake:LookAt(Vector3.new(-289.29, 1768.14, -7.256))
							Jake:SayFor(2, "Please go Tess.", 2, "This is not up for discussion.", 1.5, "You must leave.")
							wait(0.1)
							spawn(function()
								Jake:LookAt(Vector3.new(-269.403, 1772.124, 8.067))
							end)
							...
							wait(1)
							Tess:SayFor(2, "The portal...", 2, "It nearly doubled in size!", 2, "This can't be good!")
							...
							Jake:SayFor(2, "This is exactly what I feared would happen.", 2, "The portal is only going to keep getting worse.", 2, "You all need to run, now!")
							spawn(function()
							...
							Mom:SayFor(2, "Sweetie, have you been flossing?")
							Mom:SayFor(2, "Honey, that's hardly an important thing to be discussing at this time.")
							...
							Tess:SayFor(2, "Jake, you have to stop trying to do this alone.", 2, "Friends like us are always there for each other.", 2, "We can finish this together if we work as a team.")
							...
							local CrateSlamminJake = newChunk.map.RiggedCrate.Crate.Crate
							Utilities.Tween(1, "easeOutCubic", function(p760)
								CrateSlamminJake.CFrame = newChunk.map.CratePos.CFrame:Lerp(newChunk.map.CratePos1.CFrame, p760)
							end)
							wait(0.5)
							Utilities.Tween(1, "easeOutCubic", function(p761)
								CrateSlamminJake.CFrame = newChunk.map.CratePos1.CFrame:Lerp(newChunk.map.CratePos3.CFrame, p761)
							end)
							wait(1)
							Utilities.Tween(0.7, "linear", function(p762)
								CrateSlamminJake.CFrame = newChunk.map.CratePos3.CFrame:Lerp(newChunk.map.CratePos4.CFrame, p762) * CFrame.Angles(4 * p762, 0, 0)
							end)
							spawn(function()
							...
							spawn(function()
								...
								Utilities.Tween(4, "linear", function(p763)
									CrateSlamminJake.CFrame = newChunk.map.CratePos4.CFrame:Lerp(newChunk.map.CratePos5.CFrame, p763) * CFrame.Angles(6 * p763, 0, 0 * p763)
								end)
							end)
							spawn(function()
								wait(2.10)
							...
							delay(1, function()
								...
								CrateSlamminJake.CFrame = CFrame.new(0, 0, 0)
							...
							spawn(function()
								Utilities.Tween(5, "linear", function(p768)
									CrateSlamminJake.CFrame = newChunk.map.CratePos6.CFrame:Lerp(newChunk.map.CratePos7.CFrame, p768)
								end)
							...
							Dad:SayFor(2, "".. _p.PlayerData.trainerName .. ", it's up to you!", 2, "Stop Hoopa now, before it creates another portal!")
							...
							spawn(function()
								...
								spawn(function()
									...
									spawn(function()
										...
										Utilities.lookAt(CFrame.new(-318.808, 1775.139, 19.008, 0.99796021, -0.0275559761, 0.0575866699, -0, 0.902045727, 0.431640625, -0.0638400912, -0.430760175, 0.900205612))
										spawn(function()
										...
										delay(.75, function()
											...
											spawn(function()
												MasterControl:WalkTo(Vector3.new(-311.419, 1768.082, 7.895))
											end)									
											spawn(function()
		...
		onLoad_gym8 = function(chunk)        
			local doneGymBefore = false	

			if (_p.BitBuffer.GetBit(_p.PlayerData.defeatedTrainers, 231) or _p.BitBuffer.GetBit(_p.PlayerData.defeatedTrainers, 232)) and _p.PlayerData.badges[8] then
				local capnpc = _p.NPC:new(chunk.map.Room6.CaptainNPC)
				doneGymBefore = true

				capnpc.model.Head.face.Transparency = 0
				for _, capon in capnpc.model:GetChildren() do
					if capon:IsA('BasePart') then
						capon.Transparency = 0
						capon.Anchored = true
					end
				end
				capnpc.model.HumanoidRootPart.Transparency = 1

				interact[capnpc.model] = function()
					capnpc:Say("The league is where Roria's toughest trainers meet together and battle it out in a test of strength and tactics.",
						"The winner is named the champion of all Roria.",
						"Imagine that, huh.",
						"Maybe it'll be you.",
			...
			_p.Surf:forceUnsurf()
			local m = chunk.map
			chunk.npcs["Pirate Smitty"].model.HumanoidRootPart.Anchored = true
			chunk.npcs["Pirate Mary"].model.HumanoidRootPart.Anchored = true
			chunk.npcs["Pirate Peterson"].model.HumanoidRootPart.Anchored = true
			-- set npcs in room

			chunk.npcs["Pirate Mary"].model.Parent = m.Room2
			chunk.npcs["Pirate Smitty"].model.Parent = m.Room3
			chunk.npcs["Pirate Peterson"].model.Parent = m.Room5
			chunk.npcs["Pirate Mary"].model.HumanoidRootPart.BodyGyro:Destroy()
			chunk.npcs["Pirate Mary"].model.HumanoidRootPart.BodyPosition:Destroy()
			chunk.npcs["Pirate Smitty"].model.HumanoidRootPart.BodyGyro:Destroy()
			chunk.npcs["Pirate Smitty"].model.HumanoidRootPart.BodyPosition:Destroy()
			chunk.npcs["Pirate Peterson"].model.HumanoidRootPart.BodyGyro:Destroy()
			chunk.npcs["Pirate Peterson"].model.HumanoidRootPart.BodyPosition:Destroy()

			_p.DataManager:loadModule('Gym8')

			local g = _p.DataManager:getModule('Gym8')
			...
			local exitFunc = function()
				...
				Utilities.FadeOut(1)       
				_p.DataManager.currentChunk:destroy()
				local newc = _p.DataManager:loadChunk('chunk79')
				spawn(function()    
					local cam = workspace.CurrentCamera
					local doorPart = newc.map['CaveDoor:gym8']
					local doorCF = doorPart.CFrame
					local pcf = doorCF + Vector3.new(8,-5,-6)
					Utilities.Teleport(pcf)
					_p.Surf:forceSurf(pcf)
					_p.MasterControl:LookAt(doorCF + Vector3.new(22,-5,-20))
					local op =  CFrame.new(0, -doorPart.Size.Y/2+3, 0)
					local of = doorCF * op.p

					cam.CoordinateFrame = CFrame.new(of + (doorCF * CFrame.Angles(math.rad(20), 0, 0)).lookVector*20, of)
					spawn(function() Utilities.FadeIn(1) end)
					MasterControl.WalkEnabled = true

					--    _p.Surf:surf(doorPart.CFrame * CFrame.new(0, -doorPart.Size.Y/2+3, -doorPart.Size.Z-4))
				end)
			end
			...
			touchEvent(nil, m.Room6.gymExit, true, function()
				exitFunc()
			end)
			touchEvent(nil, m.Room3.gymExit, true, function()
				exitFunc()
			end)
			touchEvent(nil, m.Room6.TRIGGER, true, function()
				if doneGymBefore then return end  --old done

				local chair = m.Room6.Chair
				local cm = chair.Main
				local sk = chair.Skeleton.Torso
				local capt = chair.Captain.Torso
				local cmcf = cm.CFrame
				local doUpDWN = false
				local parts = {}
				local inv = cm.CFrame:inverse()
				for _, p in pairs(Utilities.GetDescendants(chair, 'BasePart')) do
					if p ~= cm then
						parts[p] = inv * p.CFrame
					end
				end

				for _, skele in chair.Skeleton:GetChildren() do
					if skele:IsA('BasePart') then
						local newW = Instance.new('WeldConstraint')
						newW.Parent = cm
						newW.Part0 = cm
						newW.Part1 = skele
						skele.Anchored = false
					end
				end
				for _, capn in chair.Captain:GetChildren() do
					if capn:IsA('BasePart') then
						local newW = Instance.new('WeldConstraint')
						newW.Parent = cm
						newW.Part0 = cm
						newW.Part1 = capn
				...
				task.spawn(function() _p.Menu:disable() end)
				_p.MasterControl:Stop()
				_p.MasterControl.WalkEnabled = false
				_p.MasterControl:WalkTo(m.Room6.WalkTo.CFrame)
				task.spawn(function()
					local cam = workspace.CurrentCamera
					cam.CameraType = Enum.CameraType.Scriptable
					cam.CFrame = m.Room6.Cam.CFrame
				end)
				task.wait(0.25)

				Utilities.Tween(1.5, nil, function(a)
					Utilities.MoveModel(cm, cmcf * CFrame.new(0, 0.5 * a, 0))
				end)

				cmcf = cm.CFrame

				Utilities.Tween(0.5, nil, function(a)
					Utilities.MoveModel(cm, cmcf * CFrame.Angles(0, math.rad(180 *a) , 0))
				end)

				cmcf = cm.CFrame

				for _, skeleton in chair.Skeleton:GetChildren() do
					if skeleton:IsA('BasePart') then
						skeleton.Transparency = 0
					end
				end
				...
				cmcf = cm.CFrame
				for _, skellers in chair.Skeleton:GetChildren() do
					if skellers:IsA('BasePart') then
						skellers.Transparency = 1
					end
				end
				for _, capn in chair.Captain:GetChildren() do
					if capn:IsA('BasePart') then
				...
				chair.Captain.HumanoidRootPart.Transparency = 1
				chair.Captain.Head.face.Transparency = 0
				Utilities.Tween(0.5, nil, function(a)
					Utilities.MoveModel(cm, cmcf * CFrame.Angles(0, -math.rad(180 *a) , 0))
				end)
				cmcf = cm.CFrame
				...
				task.spawn(function()
					local st = tick()
					while doUpDWN == true do
						local et = (tick()-st)*1
						local cf = cm.CFrame + Vector3.new(0, math.sin(et)*.005, 0)
						cm.CFrame = cf
						for p, rcf in pairs(parts) do
							p.CFrame = cf * rcf
						end
				...
				chat:say(chair.Captain,"Hehehe, did that scare you?",
					"Hmph, you don't took that scared.",
				...
				task.wait(0.5)
				print('does happen before?')
				local win = _p.Battle:doTrainerBattle {
					PreventMoveAfter = true,
					trainerModel = m.Room6.capnBattleM,
					vs = {name = 'Captain B', id = 8128548077,  hue = 10000, sat = 10000},
					num = 223
				}
				if win then
					local capnpc = _p.NPC:new(chunk.map.Room6.CaptainNPC)
					...
					capnpc.model.HumanoidRootPart.Transparency = 1
					chair.Captain.Head.face.Transparency = 1
					for _, capn1 in chair.Captain:GetChildren() do
						if capn1:IsA('BasePart') then
							capn1.Transparency = 1
						end
					end

					capnpc:Say("A pirate always has a hard time parting ways with their treasure, but I cannot deny someone who has earned one of these.",
						"I want you to have this badge in honor of your win today.")

					badgeSpin(chunk.map.Room6.Badge8, 'Haunted')

					capnpc:Say("I also want you to have this TM.",
					...
					chat.bottom = false
					capnpc:Say("You don't seem like the kind of kid that causes a lot of trouble, so I don't expect to see you around here anymore.", 
		...
		onLoad_chunk92 = function(chunk)

			local cress = chunk.map.Cressilia_Bronze_Legacy
			local moon = chunk.map.CrescentMoon.Moon

			if completedEvents.CresseliaBattle then
				if cress then
					cress:Destroy()
				end
				if moon then
			...
			touchEvent('CresseliaBattle', chunk.map.SceneTrigger, true, function()
				if completedEvents.CresseliaBattle then
					return
				end
				...
				local cam1 = CFrame.new(-688.42, 84.697, 151.54) * CFrame.Angles(0, math.rad(150), 0)
				local cam2 = CFrame.new(-679.75, 97.176, 133.834) * CFrame.Angles(math.rad(10), math.rad(150), 0)
				local cam3 = CFrame.new(-680.572, 84.483, 177.091) * CFrame.Angles(math.rad(-10), math.rad(40), 0)
				...
				Utilities.Teleport(CFrame.new(-688.42, 84.697, 151.54) * CFrame.Angles(0, math.rad(150), 0))

				local moonModel = chunk.map.CrescentMoon.Moon
				cam.CFrame = cam1
				task.wait(2)

				Tween(3, 'easeInOutCubic', function(a)
					cam.CFrame = cam1:Lerp(cam2, a)
				end)

				-- moon
				if moonModel then
					moonModel.CFrame = moonModel.CFrame * CFrame.Angles(0, 0, math.rad(90))
					local startMoonCFrame = moonModel.CFrame
					Tween(3, 'easeInOutCubic', function(a)
						moonModel.CFrame = startMoonCFrame + Vector3.new(0, -0.11 * a, 0)
					end)
				end
				...
				Tween(3, 'easeInOutCubic', function(a)
					cam.CFrame = cam2:Lerp(cam3, a)
				end)

				-- cress
				if cress then
					local startCFrame = cress:GetPivot()
					local endCFrame = CFrame.new(-704.217, 88.212, 179.805) * CFrame.Angles(0, math.rad(150), 0)

					Tween(5, 'easeInOutCubic', function(a)
						cress:PivotTo(startCFrame:Lerp(endCFrame, a))
					end)
				end
				...
				if not character or not character.Parent then
					warn("Character not found for cutscene animation")
				else
					local humanoid = character:FindFirstChildOfClass("Humanoid")

					if not humanoid then
						warn("Humanoid not found for cutscene animation")
					else
						...
						animation.AnimationId = "rbxassetid://10840108891"
				...
				Tween(3, 'easeInOutCubic', function(a)
					cam.CFrame = cam3:Lerp(cam1, a)
				end)

				_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Cresselia, {
					musicId = {110878764878458},
					musicVolume = 4
				})
