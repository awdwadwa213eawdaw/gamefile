return function(_p)
	...
	function BattleClient:doWildBattle(encounter, battle, repelEnabled)
		...
		if repelEnabled and encounter.ld then
		...
		wait(.5)
		while not battle.setupComplete do
			stepped:wait()
		end
		battle:focusScene()
		Utilities.FadeIn(.5)
		...
		local didPlayerWin = battle.playerWon == true
		...
		if not blackout then
			cam.CoordinateFrame = battle.afterBattleCameraCFrame or preBattleCameraCFrame
		end
		if _p.DataManager.currentChunk.indoors then
	...
	function BattleClient:doTrainerBattle(data)
		...
		if not data.musicId then
			data.musicId = Audios.TrainerMusic
		end
		...
		if data.vs then
			...
			local ballImage = Utilities.Create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://'..(data.IconId or (data.trainer and data.trainer.IconId) or 13045953722),
	...
	function BattleClient:blackedOut(wild, moneyLost)
		...
		if chunk.id == 'chunk1' then
			...
			Utilities.Teleport(CFrame.new(3+math.random(-20, 20), 86, 389+math.random(-20, 20))) --TeleportToSpawnBox()
			if blackOutTo then
			...
			stepped:wait()
			_p.MusicManager:prepareToStack(0)
			_p.MusicManager:stackMusic(10841090901, 'RoomMusic')
			Utilities.FadeIn(1)
			...
			_p.DataManager.ignoreRegionChangeFlag = nil
			local exit = room.model.Exit
			exit.Touched:connect(function(p)
				wait(2.5)
				_p.Events.onBlackoutFailsafe()
			end)	
		end
	end
	...
	function BattleClient:doVsAnimation(name, imageId, hue, sat, val)
		...
		local v = create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://13051409086', --11226844934
	...
	function BattleClient:doPVPBattle(data)
		if self.currentBattle then return end
		local battlethemes = {
			Audios.DefaultPvpTheme, -- Default PVP Theme
			Audios.MummyBattle, -- Mummy Battle
			Audios.RivalBattle, -- Rival Battle
			Audios.TrainerBattleTheme, -- Trainer Battle Theme
			Audios.VictiniBattle, -- Victini Battle
			Audios.Gym4LeaderFight, -- Gym 4 Leader Fight
			Audios.Gym3LeaderFight, -- Gym 3 Leader Fight
			Audios.Gym2LeaderFight, -- Gym 2 Leader Fight
			Audios.Gym1LeaderFight, -- Gym 1 Leader Fight
			Audios.RedGoldBattle, -- Red VS Gold Theme
			Audios.CynthiaBattle, -- Cynthia Battle Theme
			Audios.XYBattleTheme, -- XY Battle Theme
			Audios.SmBattleTheme, -- SM Battle Theme
			Audios.SWSHBattleTheme, -- SWSH Battle Theme
			Audios.BDSPBattleTheme, -- BDSP Battle Theme
			Audios.SVBattleTheme, -- SV Battle Theme
		}
		MasterControl.WalkEnabled = false
		MasterControl:Stop()
		MasterControl:Hidden(true)
		...
		if not data.musicId then
			data.musicId = battlethemes[math.random(1, #battlethemes)]
		end
		...
		_p.MusicManager:stackMusic(10841096549, 'TeamPreview', .3)
	...
	function BattleClient:do2v2Battle(data)
		if self.currentBattle then return end
		local battlethemes = {
			10841095807, -- Default PVP Theme
			10842179046, -- Mummy Battle
			10841926945, -- Rival Battle
			10841056335, -- Trainer Battle Theme
			10842216951, -- Victini Battle
			10842058815, -- Gym 4 Leader Fight
			10841945346, -- Gym 3 Leader Fight
			10841966565, -- Gym 2 Leader Fight
			10841935520, -- Gym 1 Leader Fight
			12208821971, -- Red VS Gold Theme
			12208828121, -- Cynthia Battle Theme
			11229270931, -- XY Battle Theme
			11229272877, -- SM Battle Theme
			12208824207, -- SWSH Battle Theme
			12208832379, -- BDSP Battle Theme
			12208826076, -- SV Battle Theme
		}
		MasterControl.WalkEnabled = false
		MasterControl:Stop()
		MasterControl:Hidden(true)
...
--]]
	...
	function BattleClient:fulfillRequest()
		...
		if request.requestType == 'move' then
			...
			while i <= nActive do
				...
				if a and not a.fainted then
					if a.teamn and a.teamn ~= self.teamn then
						if self.npcPartner then
							local enabledMoves = {}
							for mi, m in pairs(a.moves) do
								if not m.disabled and (not m.pp or m.pp > 0) then
									table.insert(enabledMoves, mi)
								end
							end
							...
							-- temp 2v2 npc client-decided move
							choices[i] = 'move ' .. enabledMoves[math.random(1, #enabledMoves)] .. ' ' .. math.random(1, 2) .. mega
						else
							choices[i] = 'notmyhalf'
						end
					else
						...
						if moveLocked and a.trapped then
							...
							for j = 1, i - 1 do
								pcall(function()
									alreadySwitched[tonumber(choices[j]:match('^switch (%d)$'))] = true
								end)
								pcall(function()
									if choices[j]:find(' mega') then
										alreadyChoseMega = true
									end
								end)
								pcall(function()
									if choices[j]:find(' zmov') then
										zMoveUsed = true
									end
								end)
								pcall(function()
									if choices[j]:find(' ultra') then
										alreadyChoseUltra = true
									end
								end)
								pcall(function()
									if choices[j]:find(' dynamax') then
										alreadyChoseDmax = true
									end
								end)
							end
			...
			spawn(function()
				battleGui:toggleRemainingPartyGuis(false)
			end)
		...
		elseif request.requestType == 'switch' then
			...
			if request.foeAboutToSendOut then
				--print(self.p2.name, request.foeAboutToSendOut, _p.monername)
				if self:message('[y/n]' .. self.p2.name .. ' is about to send in ' .. request.foeAboutToSendOut .. '. Will you switch your pokemon?') then
					spawn(function()
						if not battleGui:switchPokemon() then
							self.InputChosen:fire()
						end
					end)
					local choice = self.InputChosen:wait()
					if choice then
				...
				for i = 1, switches do
					...
					spawn(function()
						battleGui:switchPokemon(fs, chooseSlot, alreadySwitched)
					end)
	...
	function BattleClient:setupScene()
		...
		if self.kind ~= 'pvp' and self.kind ~= '2v2' then
			local musicId = self.musicId or {Audios.NormalEncounterTheme, Audios.NormalEncounterThemeLoop} -- old: {424133853, 424134350}, really old: 282237556
	...
	function BattleClient:winner(winner)
		self.finalWinner = winner
		self.playerWon = tostring(winner) == "1"

		self.done = true
		if winner then
			if self.kind == 'trainer' and winner == '1' then
				local trainer = self.trainerModelObj
				local root = trainer.Root
				local parts = {}
				for _, p in pairs(trainer.Model:GetChildren()) do
