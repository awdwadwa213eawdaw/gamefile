...
return function(_p)
	...
	local formats = {"OU", "UU","Ubers","UUbers", "AG"}
	...
	do
		...
		function pvp:acceptRequest(request)
			...
			local Eligible
			local reason
			if request.Format and request.Format ~= 'AG' then
				Eligible,reason = _p.Network:get('PDS', 'CheckBanlist', request.Format)
				if not Eligible then
					_p.NPCChat:say(reason)	
					return end
			end	
			_p.Network:get('PDS', 'setFormat', request.Format)
	...
	function pvp:sendRequest()
		...
		local Eligible	
		local reason
		if settings.Format and settings.Format ~= 'AG' then
			Eligible,reason = _p.Network:get('PDS', 'CheckBanlist', settings.Format)
			if not Eligible then
				_p.NPCChat:say(reason)
				return end
		end
		_p.Network:get('PDS', 'setFormat', settings.Format)
	...
	function pvp:onClickedPlayer(player)
		...
		if not battleRQgui then
			...
			--makeOptions({'OU','Ubers','AG'}, {'OU','Ubers','AG'}, 'Format', 1) {
			--		Size = UDim2.new(0.4, 0, 0.1, 0),
			...
			-- BEGIN PVP DROPWDOWN
			local formatFrame = create 'Frame' {
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, 0.45, 0),
				Position = UDim2.new(0.49, 0, 0.275, 0),
				ZIndex = 4
			}
			local formatButton = create 'ImageButton' {
				AutoButtonColor = false,
				BorderSizePixel = 0,
				BackgroundColor3 = convertColor(BrickColor.new('Deep blue').Color),
				...
				MouseButton1Click = function()
					...
					local list = create 'ScrollingFrame' {
						...
						Position = UDim2.new(0.29, 0, 0.465, 0),
					...
					local contentRelativeSize = #formatOptions * h * container.AbsoluteSize.X / list.AbsoluteSize.Y
					...
					for i, tier in pairs(formatOptions) do
						local isCurrent = settings.Format == tier
						local button = create 'ImageButton' {
							...
							MouseButton1Click = function()
								...
								settings.Format = tier
								formatFrame:ClearAllChildren()
								write(tier) {Frame = formatFrame, Scaled = true}
							end
						}
						write(tier) {
							Frame = create 'Frame' {
			...
			formatFrame.Parent = formatButton
			write 'AG' {Frame = formatFrame, Scaled = true}
