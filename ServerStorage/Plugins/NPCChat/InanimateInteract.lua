return function(_p)
	...
	return {
		...
		--	_p.DataManager:preload(576438342, 11631932298)
		...
		ShayminEncounter = function(model)
			...
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(enc, {musicId = Audios.LegendsBattle})
		...
		vicSeal = function(model)
			...
			_p.MusicManager:stackMusic(10842216951, 'BattleMusic', .4)
		...
		Landorus = function(model)
			...
			spawn(function() _p.Battle._SpriteClass:playCry(nil, {id = 10840606887, startTime = 104.50, duration = 1.68}) end)
			wait(1.7)
			chat:manualAdvance()
			wait(.5)
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Landforce, {musicId = Audios.LegendsBattle})
		...
		heatran = function(model)
			...
			wait(.5)
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Heat, {musicId = Audios.LegendsBattle})
		...
		diancie = function(model)
			...
			wait(1.8)
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Jewel, {musicId = Audios.LegendsBattle})
		...
		Articuno = function(model)
			...
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(enc, {musicId = Audios.LegendsBattle})
			_p.NPCChat:say('Articuno can now be found roaming in the wild.')
			_p.MasterControl.WalkEnabled = true
			_p.MasterControl:Stop()
		end,
		...
		Zapdos = function(model)
			...
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(enc, {musicId = Audios.LegendsBattle})
			_p.NPCChat:say('Zapdos can now be found roaming in the wild.')
			_p.MasterControl.WalkEnabled = true
			_p.MasterControl:Stop()
		end,
		...
		Moltres = function(model)
			...
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(enc, {musicId = Audios.LegendsBattle})
			_p.NPCChat:say('Moltres can now be found roaming in the wild.')
			_p.MasterControl.WalkEnabled = true
			_p.MasterControl:Stop()
		end,

		MarshadowShadow = function(model)
			_p.ShadowVoid:doBattleClick(workspace:FindFirstChild('ShadowVoid'))
		end,
		...
		GPedistal = function(model)
			...
			if choice then
				...
				sceneMusic.Volume = 0
				spawn(function()
					_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.LavaBeast, {
						battleSceneType = 'LavaCave',
						musicId = Audios.LegendsBattle
						--						afterBattleCameraCFrame = cam.CFrame
					})
					_p.MasterControl.WalkEnabled = false
		...
		SafariBerries = function(model)
			local chunk = _p.DataManager.currentChunk
			if not chunk or chunk.id ~= 'chunk76' then return end
			if not _p.MasterControl.WalkEnabled then return end
		...
		MasterSword = function(model)
			...
			local SwordCFrame = SwordMain.CFrame
			local cam = workspace.CurrentCamera
			local camPos = chunk.map.CAMERA.CFrame
			local telePos = CFrame.new(-98.561, -249.909, 2660.251)
			...
			cam.CameraType = Enum.CameraType.Scriptable
			spawn(function()
				Utilities.Teleport(telePos)
				_p.MasterControl:LookAt(CFrame.new(-90.195, -244.565, 2660.75))
			end)
			cam.CFrame = camPos
			_p.NPCChat:say(strengthen .. ' used Strength!')
			Utilities.Tween(1, "easeInSine", function(a)
				--            model.Main.CFrame = model.Main.CFrame * CFrame.Angles(math.rad(0.5-a), 0, 0)
				Utilities.MoveModel(SwordMain, SwordCFrame + Vector3.new(0, 2*a, 2*a))
		...
		EclipseGate = function(model)
			...
			if _p.PlayerData.completedEvents.DefeatHoopa then
				...
				if choice then
					...
					Utilities.Tween(3, "easeInSine", function(a)
						Utilities.MoveModel(GateMain, GateCFrame + Vector3.new(a * 9, 0, 0))
						Utilities.sound(9120878109, 1, nil, nil)
					end)
				else
		...
		Gym8S = function(model)
			...
			spawn(function()
				local npcs = _p.DataManager.currentChunk.npcs
				if npcs.CaptainNPC then
					local npc = npcs.CaptainNPC
					...
					npc.model.Head.face.Transparency = 1
					npc.model.HumanoidRootPart.Transparency = 1
				end  
			end)
			local g = _p.DataManager:getModule('Gym8')
			Utilities.FadeOut(0.5, Color3.new(0, 0, 0))
			local main = model.Main
			if model.Parent.Name == 'Room4' then
				_p.Menu:disable()
				g:unfoldRoom(0.0001, 1)
			elseif model.Parent.Name == 'Room2' then
				_p.Menu:disable()
				g:unfoldRoom(0.0001, 2)
			end
