...
--			ch.Position = UDim2.new(ch.Position.X.Scale-.25, 0, ch.Position.Y.Scale, 0)
	...
	function Menu:enable()
		...
		if not container then
			...
			container = create 'ImageLabel' { -- 184 x 494; tab is 47px wide; border is about 4px thick
				...
				Image = 'rbxassetid://6282604603',-- 313105946 -- 281083812
				...
				create 'ImageButton' {
					...
					create 'ImageButton' { -- 150x75
						Name = 'Party',
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://5879331096',
						...
						create 'ImageLabel' {
							BackgroundTransparency = 1.0,
							Image = 'z',
							ImageColor3 = Color3.new(.45, .6, 1),--Color3.new(1, 0, 0),--Color3.new(.4, .1, .1),--.2, .6, 1),
							SizeConstraint = Enum.SizeConstraint.RelativeYY,
					...
					create 'ImageButton' {
						Name = 'Bag',
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://5879325229',
						SizeConstraint = Enum.SizeConstraint.RelativeXX,
						Size = UDim2.new(0.73, 0,0.41, 0),
						Position = UDim2.new(0.115, 0,0.35, 0),
						ZIndex = 6,
						MouseButton1Click = function()
							if not self.enabled then return end
							spawn(function() self:disable() end)
					...
					create 'ImageButton' {
						Name = 'Trainer',
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://5879326420',
						SizeConstraint = Enum.SizeConstraint.RelativeXX,
						Size = UDim2.new(0.825, 0,0.4, 0),
						Position = UDim2.new(0.06, 0,0.51, 0),
						ZIndex = 6,
						MouseButton1Click = function()
							if not self.enabled then return end
					...
					create 'Frame' {
						...
						BackgroundTransparency = 1.0,
						Size = UDim2.new(0.0, 0, 0.028, 0),
						Position = UDim2.new(0.5, 0, 0.627, 0),
						ZIndex = 6,
					},
					...
					create 'ImageButton' {
						Name = 'Save',
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://5879323125',
						SizeConstraint = Enum.SizeConstraint.RelativeXX,
						Size = UDim2.new(0.825, 0,0.4, 0),
						Position = UDim2.new(0.06, 0,0.67, 0),
						ZIndex = 6,
						MouseButton1Click = function()
							if not self.enabled then return end
							spawn(function() self:disable() end)
							...
							if _p.NPCChat:say('[y/n]Would you like to save the game?') then 
								if not self:saveGame() then
									_p.NPCChat:say('Please wait a bit and try again.')
								end
							end
							MasterControl.WalkEnabled = true
							self:enable()
					...
					create 'ImageButton' {
						Name = 'Options',
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://8975798622', -- Options Icon
						SizeConstraint = Enum.SizeConstraint.RelativeXX,
						Size = UDim2.new(0.825, 0,0.4, 0),
						Position = UDim2.new(0.06, 0,0.83, 0),
						ZIndex = 6,
						MouseButton1Click = function()
	...
	function Menu.party:open(image)
		...
		if not backdrop then
			...
			local function makeSlot(num)
				...
				local function battleSelectPokemon()
					...
					if self.battleSelectMenuDestroy then
						self.battleSelectMenuDestroy()
					end
					...
					self.battleSelectMenuDestroy = function()
						switch:Destroy()
						summary:Destroy()
					end
		...
		self.cancel.BackgroundColor3 = self.battleEvent and BrickColor.new('Dark green').Color or BrickColor.new('Deep blue').Color
		backdrop.Image = 'rbxassetid://'..(self.battleEvent and 5262067210 or 5262067210)
	...
	function Menu.party:close()
		...
		if self.battleSelectMenuDestroy then
			self.battleSelectMenuDestroy()
			self.battleSelectMenuDestroy = nil
		end
	...
	function Menu.party:viewSummary(pokemon, ignoreParty, disableMoveSwitching) -- OVH  todo: EV/IV Viewer
		...
		local main = create 'ImageLabel' {
			Name = 'Summary',
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://113262629023349', --Image = 'rbxassetid://6809938118',
			ImageColor3 = Color3.new(1, 1, 1), --ImageColor3 = Color3.new(.6, .6, .8),
			SizeConstraint = Enum.SizeConstraint.RelativeYY,
		...
		write(pokemon.nickname or name) {
			Frame = create 'Frame' {
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, 0.057, 0),
				Position = UDim2.new(ballIcon and .09 or 0.04, 0, 0.028, 0),
		...
		if pokemon.egg then
			...
			for i, s in pairs({'Attack', 'Defense', 'Sp. Atk', 'Sp. Def', 'Speed'}) do
				...
				write(s) {
					Frame = create 'Frame' {
						Size = UDim2.new(0.0, 0, 0.095, 0),
						Position = UDim2.new(0.05, 0, 0.2*(i-1)+.05+.005, 0),
				...
				write(tostring(pokemon.stats[i])) {
					Frame = create 'Frame' {
						Size = UDim2.new(0.0, 0, 0.095, 0),
						Position = UDim2.new(0.95, 0, 0.2*(i-1)+.05+.005, 0),
						ZIndex = 8, Parent = statsRF.gui, BackgroundTransparency = 1.0,
					}, Scaled = true, TextXAlignment = Enum.TextXAlignment.Right,
				}
			end
			...
			local effortViewer = create 'ImageButton' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6604588261', -- Hexagon 
			...
			write 'Item:' {
				Frame = create 'Frame' {
					Size = UDim2.new(0.0, 0, 0.029, 0),
					Position = UDim2.new(0.047, 0, 0.72, 0),
					ZIndex = 6, Parent = main, BackgroundTransparency = 1.0,
				}, Scaled = true, TextXAlignment = Enum.TextXAlignment.Left,
			}
			write(pokemon.itemName or 'None') {
				Frame = create 'Frame' {
					Size = UDim2.new(0.0, 0, 0.029, 0),
					Position = UDim2.new(0.4525, 0, 0.72, 0),
					ZIndex = 6, Parent = main, BackgroundTransparency = 1.0,
				}, Scaled = true, TextXAlignment = Enum.TextXAlignment.Right,
			}
			write 'Ability:' {
			...
			write('Lv '..pokemon.level) {
				Frame = create 'Frame' {
					...
					Position = UDim2.new(0.78, 0, 0.035, 0),
					ZIndex = 6, Parent = main, BackgroundTransparency = 1.0,
				}, Scaled = true, TextXAlignment = Enum.TextXAlignment.Left,
			}
			spawn(function()
			...
			local iconPosX = 0.91
			local function addIcon(image, data, hText)
				local r = nil
				...
				if type(image) == "string" then
					local dat = {
						...
						Position = UDim2.new(iconPosX, 0, 0.5, 0),
						Parent = main
					}
					...
					dat.Image = image
					dat.ZIndex = 10
					r = create("ImageButton")(dat)
				elseif typeof(image) == "Instance" then
					if not data then
						image.Size = UDim2.new(0.05, 0, 0.05, 0)
					end					
					image.Position = UDim2.new(iconPosX, 0, 0.5, 0)
					image.ZIndex = 10
					image.Parent = main

					r = image
				elseif image.sprite then

					image.sprite.buttton = true

					local anim = _p.AnimatedSprite:new(image.sprite)
					local s = anim.spriteLabel
					anim:Play()
					s.Size = image.size or UDim2.new(0.08,0,0.08,0) -- 0.9 = 1
					s.ImageColor3 = image.color or Color3.fromRGB() 
					s.Position = UDim2.new(iconPosX, 0, 0.5, 0) -- 0.6 to 0.5
					s.ZIndex = 10
					s.Parent = main
				...
				if hText then
					_p.ToolTip:create(r, hText) 
				end

				iconPosX = iconPosX - 0.057
				if r then return r end
			end

			if pokemon.shiny then
				addIcon("rbxassetid://5879636660", {
			...
			if pokemon.trade then				
				addIcon("rbxassetid://135900310819269", {
					ImageColor3 = Color3.fromRGB(255, 255, 255)
				}, "Trade Locked")
			end

			if pokemon.event then
				addIcon("rbxassetid://7551989094", {
					ImageColor3 = Color3.fromRGB(239, 184, 56),
				}, "Event")
			end

			for i, t in pairs(_p.Pokemon:getTypes(pokemon.types)) do
				local rf = roundedFrame:new {
					Name = 'TypingRoundedFrame',
				...
				write (t) {
					Frame = create 'Frame' {
						Parent = rf.gui, BackgroundTransparency = 1.0,
			...
			spawn(function()
				...
				local ot, id = _p.Pokemon:getOT(id)
				if not ot or not id or not main.Parent then return end
				...
				if pokemon.otEffect then
					table.insert(pokemon.otEffect, 2, b.Labels)
					Utilities.WriteSFX(unpack(pokemon.otEffect))
				end
			...
			write('Characteristic: '..pokemon.desc) {
				Frame = create 'Frame' {
					Size = UDim2.new(0.405, 0, 0.029, 0), --0.325
			...
			local moveContainer; moveContainer = create 'ImageLabel' { -- 250 x 347
				...
				create 'TextButton' { -- tl 137, 22; 41 x 141
					...
					create 'Frame' {
						Name = 'ButtonText',
						BackgroundTransparency = 1.0,
						Size = UDim2.new(0.0, 0, 0.51, 0),
						Position = UDim2.new(0.5, 0, 0.23, 0),
						ZIndex = 4,
					}
				}
			}
			...
			local function showDetails(rf, instant)
				...
				if move and not justClose and not instant then
					details = roundedFrame:new {
						...
						create 'ImageLabel' {
							BackgroundTransparency = 1.0,
							Image = 'rbxassetid://'..({Status=11608302974,Special=11608297053,Physical=11608295117})[move.category],
				...
				for i = 1, 4 do
					...
					local function mouseUp2()
						...
						if not dragged then
							...
							for _, mon in pairs(self.partyData) do
								if mon.name == pokemon.name then
									partyindex = mon.index
								end
							end

							local sendto = Utilities.deepcopy(pokemon.moves)

							for index, move in pairs(sendto) do		
								sendto[index] = Utilities.toId(move.name)
							end

							_p.Network:post('PDS', 'rearrangeMoves', partyindex, sendto)
					...
					rf.gui.MouseButton1Down:connect(function(x, y)
						...
						rf.ZIndex = 4
						local mouse = player:GetMouse()
						local cn = mouse.Button1Up:connect(mouseUp2)
						local oPos
						--[[if Utilities.isTouchDevice() then -- this breaks the script for mobile users lol
						...
						oPos = Vector2.new(mouse.X, mouse.Y) -- added
					...
					if m then
						...
						rf.BackgroundColor3 = Color3.new(c.r * 0.35, c.g * 0.35, c.b * 0.35)
						write(m.name) {
							Frame = create 'Frame' {
								BackgroundTransparency = 1.0,
								Size = UDim2.new(0.0, 0, 0.32, 0),
								Position = UDim2.new(0.5, 0, 0.1, 0),
								ZIndex = 4,
								Parent = rf.gui,
							},
						...
						rf.BackgroundColor3 = Color3.new(0.3, 0.3, 0.3)
					end
					table.insert(rframes, rf)
				end
			end
	...
	do
		local function triangle(a, b, c)
			...
			local t1 = create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://11608323921',--318776397',
			}
			local t2 = t1:Clone()
			local nodes
	...
	function Menu.party:remindMove(r, num)
		...
		for i, m in pairs(r.known) do
			...
			local rf = roundedFrame:new {
				...
				create 'ImageLabel' {
					BackgroundTransparency = 1.0,
					Image = 'rbxassetid://'..({Status=11608302974,Special=11608297053,Physical=11608295117})[m.category],
					SizeConstraint = Enum.SizeConstraint.RelativeYY,
	...
	--Menu.stamps = require(script.PBStamps)(_p)
