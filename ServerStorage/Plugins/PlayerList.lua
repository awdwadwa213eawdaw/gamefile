return function(_p)--local _p = require(script.Parent)
	...
	local gamemodeColors = {
		randomizer = Color3.fromRGB(0, 128, 255)
	}
	
	local function updateUser(u, badgeId, ownedPokemon, gamemode)
		...
		if gamemode and gamemodeColors[gamemode] then
			u.dex.TextColor3 = gamemodeColors[gamemode]
		end
	end
	...
	local function addUser(p)
		...
		local b, o, gamemode = 0, 0, 'adventure'
		pcall(function() b = p.BadgeId.Value end)
		pcall(function() o = p.OwnedPokemon.Value end)
		pcall(function() gamemode = p.gamemode.Value end)
		updateUser(u, b, o, gamemode)
	end
	...
	network:bindEvent('UpdatePlayerlist', function(playerName, badgeId, ownedPokemon, gamemode)
		local u
		...
		updateUser(u, badgeId, ownedPokemon, gamemode)
	end)
	...
	game:GetService('UserInputService').InputBegan:connect(function(inputObject)
		...
		elseif inputObject.KeyCode and inputObject.KeyCode == Enum.KeyCode.Tab then
			playerList:toggle()
		end
	end)
