return function(_p)
	...
	do
		...
		function shop:filterItems(searchText)
			...
			for _, thing in ipairs(originalItems) do
				local item = _p.DataManager:getData('Items', rc4(thing[1]))
				if item and item.name:lower():find(searchText, 1, true) then
					table.insert(filteredItems, thing)
				end
			end
		...
		function shop:clearSearch()
			searchActive = false
			if originalItems then
				self:displayFilteredItems(originalItems)
			end
		end

		function shop:displayFilteredItems(items)
			local gui = self.currentGui
			...
			if #items == 0 then
				local noItemsFrame = create 'Frame' {
			...
			for i, thing in pairs(items) do
				local button = create 'ImageButton' {
					...
					BackgroundTransparency = i%2==0 and 0.0 or 1.0,
					BorderSizePixel = 0,
				...
				Utilities.fastSpawn(function()
					...
					button.Size = UDim2.new(0.95, 0, s, 0)
					button.Position = UDim2.new(0.025, 0, s*(i-1), 0)
					...
					button.MouseButton1Click:Connect(showItemDetails)
					text.MouseButton1Click:Connect(showItemDetails)
				end)
			end
		end
		...
		function shop:buildList(gui, shopId)
			...
			Utilities.fastSpawn(function()
				...
				self:displayFilteredItems(items)
			end)
		end
	end
			...
			if not mainMenu then
				...
				write 'Buy' {
					Frame = Utilities.Create 'ImageButton' {
						...
						Position = UDim2.new(0.35, 0, 0.06, 0),
						ZIndex = 2,
						Parent = mainMenu.gui,
				...
				write 'Sell' {
					Frame = Utilities.Create 'ImageButton' {
						BackgroundTransparency = 1.0,
						Size = UDim2.new(0.6, 0, 0.15, 0),
						Position = UDim2.new(0.35, 0, 0.56, 0),
						ZIndex = 2,
						Parent = mainMenu.gui,
						MouseButton1Click = function()
							mainMenu.Visible = false
			...
			if not gui then
				...
				gui = _p.RoundedFrame:new{ -- gui == RF.gui
					...
					create 'ScrollingFrame' {
						...
						BackgroundTransparency = 1.0,
						BorderSizePixel = 0,
						Size = UDim2.new(0.95, 0, 0.525, 0),
						Position = UDim2.new(0.025, 0, 0.225, 0),
				...
				searchComponent = _p.Search:new({
					parent = gui,
					position = UDim2.new(0.025, 0, 0.14, 0),
					size = UDim2.new(0.95, 0, 0.07, 0),
					placeholder = "Search",
					zIndex = 3,
					backgroundColor = BrickColor.new('Deep blue').Color,
					debounceTime = 0.3
				})

				searchComponent.changed:connect(function(text)
					self:filterItems(text)
				end)

				searchComponent.cleared:connect(function()
					self:clearSearch()
				end)
			end
