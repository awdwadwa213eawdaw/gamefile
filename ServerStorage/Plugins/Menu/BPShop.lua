return function(_p)
	...
	do
		...
		function shop:buildList(gui, shopId)
			if not shopId then shopId = 'bp' end
			selection = nil
			gui.Details.BuyButton.Visible = false
			self:updateBP(gui) --

			local items--[[, encryptedIds]] = _p.Network:get('PDS', 'getShop', shopId)
				...
				Utilities.fastSpawn(function()
					...
					button.MouseButton1Click:connect(function()
						...
						elseif bp then
							...
							elseif bp == '200' then
								item.productId = _p.productId.TwoHundredBP
							elseif bp == '500' then
								item.productId = _p.productId.FiveHundredBP
							end
						end
						local icon
						if not bp then
