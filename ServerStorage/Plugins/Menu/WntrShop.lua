return function(_p)--local _p = require(script.Parent.Parent)--game:GetService('ReplicatedStorage').Plugins)
	...
	function WntrShop:updateMoney(gui)
	...
	do
		local selection
		function WntrShop:buySelection()
			...
			if selection.robux then
				_p.MarketClient:promptProductPurchase(_p.productId[selection.arg3].Value)
			else
				saying = true
		...
		function WntrShop:buildList(gui, shopId)
			selection = nil
			gui.Details.BuyButton.Visible = false
			self:updateMoney(gui) 

			Utilities.fastSpawn(function()
				local items = _p.Network:get('PDS', 'getShop', shopId)
				...
				for i, thing in pairs(items) do
					local button = create 'ImageButton' {
						...
						BackgroundTransparency = i % 2, -- i%2==0 and 0.0 or 1.0
						BorderSizePixel = 0,
						Size = UDim2.new(0.95, 0, s, 0),
						Position = UDim2.new(0.025, 0, s*(i-1), 0),
						ZIndex = 10, Parent = container,
					}
					Utilities.fastSpawn(function()
	...
	do
		function WntrShop:open(shopId)--, enableSelling ?
			self:buyMenu(shopId)
			return true
		end
	end
	...
	do
		local sig, gui
		function WntrShop:buyMenu(shopId)
			...
			if not gui then
				...
				gui = _p.RoundedFrame:new{ -- gui == RF.gui
					Name = 'MartGui',
					BackgroundColor3 = Color3.new(0.690196, 0.317647, 0.72549),--BrickColor.new('Cyan').Color,--Color3.new(.4, .8, 1),
					...
					_p.RoundedFrame:new {
						Name = 'TitleBar',
						BackgroundColor3 = BrickColor.new('Light reddish violet').Color,
		...
		function WntrShop:updateMoneyIfActive()
