...
return function(_p)--local _p = require(script.Parent.Parent)--game:GetService('ReplicatedStorage').Plugins)
	...
	local pc_bgs = {
		...
		6292074662, -- clouds
		-- scenery
		6292076545, -- sun n moon
		6292076687, -- volcano
		6292076792, -- beach
		6292076792, -- cave
		6292077884, -- iceberg
		6292078032, -- waterfall
		6292078205, -- city
		6292078357, -- galaxy
		6292078499, -- galaxy 2
		6292079829, -- mega charizards
	...
	local onMoreBoxesPurchased; do
		...
		write 'Get More Boxes' {
			Frame = create 'Frame' {
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, 0.4, 0), --0.45
				Position = UDim2.new(0.5, 0, 0.29, 0), -- 0.275
				ZIndex = 6, Parent = getMoreBoxesButton.gui,
			}, Scaled = true,
		}
		onMoreBoxesPurchased = function()
			getMoreBoxesButton:destroy()
			...
			if pName then
				...
				if chat:say('[y/n]Release ' .. pName .. '?') then
					if chat:say('This cannot be undone.', '[y/n]Are you sure you want to release ' .. pName .. '?') then
						...
						local dur = 1
						if ch then
							ch:TweenSizeAndPosition(UDim2.new(0.0, 0, 0.0, 0), UDim2.new(0.5, 0, 0.5, 0), nil, nil, dur, true, function() ch:Destroy() end)
						end
						local pc = pc_ui.pcData
						local r = pc.rcount + 1
						pc.rcount = r
	...
	local function selectBoxOptions(configButton)
		...
		local gui = Utilities.frontGui
		if boxOptions then
			boxOptions.Parent = Utilities.miscGui
				...
				local rf = _p.RoundedFrame:new {
					...
					MouseButton1Click = function() -- an option was clicked
						...
						if optionNum == 1 then--RENAME
							pcall(function() pcContainer.BoxDiv.BoxName:Destroy() end)
							...
							pcall(function() pcContainer.OtherBoxes.Container['Box'..boxNum..'Text']:Destroy() end)
									...
									if chat:say('[y/n]Release all pokemon in this box?') then
										if chat:say('WARNING! This cannot be undone. You are about to release '..nPokemon..' pokemon.',
											...
											for _, frame in pairs(frames) do
												...
												local ch = frame:GetChildren()[1]
												if ch then
													ch:TweenSizeAndPosition(UDim2.new(.0,0,.0,0), UDim2.new(.5,0,.5,0), nil, nil, dur, true, function() ch:Destroy() end)
												end
												local r = pc.rcount + 1
												pc.rcount = r
												pc.ch.m[tostring(p[1])] = {-1, r}
		...
		if isActive then-- table.insert(boxLabels, boxLabel) end
			...
			write(pc.bNames[tostring(n)] or ('Box '..n)) { -- top line
				Frame = create 'Frame' {
					Name = 'BoxName',
					BackgroundTransparency = 1.0,
					Size = UDim2.new(.0, 0, .06, 0),
					Position = UDim2.new(.1, 0, -.08, 0),
					ZIndex = 3, Parent = pcContainer.BoxDiv
				}, Scaled = true, TextXAlignment = Enum.TextXAlignment.Left
			}
			...
			searchButton = create("ImageButton")({
				BackgroundTransparency = 1, 
				Image = "rbxassetid://70411823570051", 
				SizeConstraint = Enum.SizeConstraint.RelativeYY, 
			...
			local configButton; configButton = create 'ImageButton' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://74483299376537',
				SizeConstraint = Enum.SizeConstraint.RelativeYY,
	...
	function pc_ui:bootUp(pcModel)
		...
		Utilities.sound(11842655027, nil, nil, 5)
			...
			local function createRow()
				...
				local function makeStrip()
					local strip = create 'ImageLabel' {
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://8938689312', -- Mons PC Sheet
				...
				function row:update(penShift, xShift)
					...
					if container.AbsolutePosition.Y + container.AbsoluteSize.Y < -100 then
						self:Destroy()
						return
					end
		...
		do
			...
			local party = create 'ImageButton' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://73023492923874',
				Size = UDim2.new(1.0, 0, 1.0, 0),
		...
		for i = 1, pc.maxBoxes do
			...
			write(pc.bNames[i_string] or ('Box '..i)) { -- above in list
				Frame = create 'Frame' {
					Name = 'Box'..i..'Text',
					BackgroundTransparency = 1.0,
					Size = UDim2.new(1.0, 0, 1/6*3/4*.9-0.012, 0),
		...
		Utilities.sound(11842658186, nil, nil, 5)
