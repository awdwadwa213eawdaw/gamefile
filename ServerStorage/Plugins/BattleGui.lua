return function(_p)
	...
	function BattleGui:updateButtonForZMove(button, move, zmove, zmoves)
		...
		if not move.effective then
			local effect = {}
			for ind,val in pairs(_p.Pokemon:getTypes(_p.Battle.currentBattle.p2.active[1].types)) do
		...
		if move.effective then
			local totalEffectiveness = 1
			for _, e in pairs(move.effective) do
				if typeof(e) == "table" then
					for _, v in pairs(e) do
						if typeof(v) == "number" then
							totalEffectiveness = totalEffectiveness * v
						end
					end
				elseif typeof(e) == "number" then
					totalEffectiveness = totalEffectiveness * e
				end
			end
			if totalEffectiveness ~= 1 then
				button.Effectiveness.Visible = true
				if totalEffectiveness < 0.3 then
					button.Effectiveness.ImageRectOffset = Vector2.new(300, 0)
				elseif totalEffectiveness < 0.6 then
					button.Effectiveness.ImageRectOffset = Vector2.new(300, 300)
				elseif totalEffectiveness > 3.5 then
					button.Effectiveness.ImageRectOffset = Vector2.new(0, 0)
				elseif totalEffectiveness > 1.5 then
	...
	function BattleGui:updateButtonForMaxMove(button, move, maxMove)
		...
		if not move or not maxMove or maxMove == '' then
			button.ImageColor3 = Color3.new(0.5, 0.5, 0.5)
			button.ImageTransparency = 0.5
			return false
		end
		...
		pcall(function()
			if not move.effective and battle and battle.yourSide and battle.yourSide.active and battle.yourSide.active[1] then
				local effect = {}
		...
		local maxMoveName = maxMove.name or maxMove.move or maxMove
		local name1 = maxMoveName
		local name2
		local len = name1:len()
		local splitPosition, dif
		local start = 1
	...
	function BattleGui:animCapture(poke, pokeballId, shakes, critical, sbCount)
		...
		poke.sprite:animCaptureAttempt(pokeballId, shakes, critical, caught, sbCount ~= nil)
		if sbCount then
	...
	function BattleGui:animAbility(poke, abilityName)
		...
		local gui = create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://'..({6604463794, 6604466114})[n], -- Outlines A & B
			Size = UDim2.new(.2, 0, (.2/292*110-0.005), 0),-- 292x110
			SizeConstraint = Enum.SizeConstraint.RelativeXX,
			Parent = Utilities.miscGui,
		}
	...
	function BattleGui:animBoost(poke, good)
		...
		Utilities.sound(good and Audios.RaiseStats or Audios.LowerStats, .3, nil, 5)
		spawn(function()
			local angles = {}
	...
	function BattleGui:animZPower(battle, poke)
		wait(0.5)
		...
		Utilities.MoveModel(main, battle.CoordinateFrame1 + Vector3.new(0, 100, 0))
		...
		if self.kind ~= '2v2' then
			...
			elseif sprite.siden == 2 then
				trainer = battle.trainerModelObj
			end
		else -- to lazy to wirte code for this. Just giving local player model for now. :/
			trainer = battle.playerModelObj
		end
		...
		--local animtrack = trainer.Humanoid:LoadAnimation(Utilities.Create("Animation")({
		--	AnimationId = "rbxassetid://" .. _p.animationId[(trainer.IsR15 and "R15_" or "") .. "ZPower"]
		--}))
		...
		Utilities.Tween(0.6, "easeOutQuart", function(alpha)
			...
			cam.CFrame = CFrame.new((cp + dir / bt) + Vector3.new(0, 0, 86 * alpha), focus) * CFrame.Angles(0, 0, -roll) + Vector3.new(0, inAir, 0) --	cam.CFrame = CFrame.new(cp + dir * bt, focus) * CFrame.Angles(0, 0, -roll) + Vector3.new(0, inAir, 0)
	...
	function BattleGui:animHit(target, source, type, soundid, effectiveness, suppressParticles)
		...
		--local to = target.sprite:WaitForChild("part").Position
		...
		local smack = create 'ImageLabel' {
			Name = 'SmackAnim',
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://6604468011', -- Smack 2
			ImageColor3 = color,
			Parent = Utilities.gui,
			ZIndex = 10
		}
		Utilities.sound(soundid or Audios.PVPNormalDamage, .75, effectiveness == 1 and .5 or .6, 5)
	...
	function BattleGui:animMove(battle, pokemon, move, targets)
		...
		for _, a in pairs(battle.actionQueue) do
			...
			local arg1 = args[1]
			--if pokemon.template.species == 'Marshadow'then
			-- 	pokemon.template.forme = 'Attack'
			-- local template = self:getSprite('Marshadow-Attack')
			-- 	pokemon:formeChange(template)
			-- 	pokemon.baseTemplate = template
			-- 	local shinyPrefix = pokemon.shiny and '_SHINY' or ''
			-- 	self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_FRONT/Marshadow-Attack')
			-- 	self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_BACK/Marshadow-Attack')                
			-- pokemon.frontSpriteOverride = require(game:GetService('ServerStorage').Data.GifData)[shinyPrefix..'_FRONT']['Marshadow-Attack']
			-- end
			...
			elseif arg1 == '-supereffective' then
				...
				if target then
					effectives[target] = 2
					soundids[target] = Audios.PVPHighDamage
				end
			elseif arg1 == '-resisted' then
				local target = battle:getPokemon(args[2])
				if target then
					effectives[target] = .5
					soundids[target] = Audios.PVPLowDamage
				end
			end
		...
		if moveAnims[move.id] then
			...
			if continue == 'sound' then -- TODO: this could be better; for now, ONLY RETURN THIS if the move has a single target
				...
				Utilities.sound(s or Audios.PVPNormalDamage, .75, e == 1 and .5 or .6, 5)
				return
			end
	...
	do -- original code
		...
		local msgBox = create 'ImageLabel' {
			Name = 'BattleMsg',
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://136790568710945', -- Outline C
		...
		local function processMessages()
			...
			local line2Pos = line1Pos + boxHeightFill/(font.baseHeight*2+font.lineSpacing)*(font.baseHeight+font.lineSpacing)+0.05
			...
			while #msgQueue > 0 do
				...
				repeat
					...
					local lf = Utilities.Create 'Frame' {
						BackgroundTransparency = 1.0,
						Size = UDim2.new(0.76, 0, lineHeight/relSize-0.1, 0),
						Position = UDim2.new(0.12, 0, ((line==1 and line1Pos+0.05 or line2Pos)-mbcOffset)/relSize, 0),
	...
	function BattleGui:mainChoices(...)
		...
		local args = {...}
		local rqPokemon, slot, nActive, moveLocked, isFirstValid, alreadySwitched, alreadyChoseMega = ...
		local battle = _p.Battle.currentBattle
		...
		self.mainButtonClicked = function(name)
			...
			if name == 'Fight' then
				if moveLocked then
					...
					self:fightChoices(self.moves, rqPokemon, slot, nActive, alreadyChoseMega)
				end
			else
				pcall(function()
				...
				if name == 'Run' then
					...
					elseif battleKind == 'wild' then
						if battle.isRaid then
							self:message("You cannot run from a raid.")
							state = 'canchoosemain'
							return
						end
						...
						self:message('You can\'t run from a Trainer battle!')
						state = 'canchoosemain'
					end
				...
				elseif name == 'Bag' or name == 'Berry' then
					if battle.isRaid then
						self:message("You cannot use your bag in a raid.")
						state = 'canchoosemain'
						return
					end
		...
		local function writeTextForBtn(label, name, textColor)
			...
			if table.find(fIcon, name) then
				create 'Frame' {
					...
					Position = name == 'Ball' and UDim2.new(0.5, 0, 0.25, 0) or UDim2.new(0.115, 0, 0.06, 0),
					Parent = label,
				}
			end
			...
			write(name) {
				Frame = create 'Frame' {
					Name = 'txt',
					BackgroundTransparency = 1.0,
					Size = UDim2.new(1.0, 0, 0.42, 0),
					Position = name  == "Ball" and UDim2.new(-0.1, 0, 0.28, 0) or UDim2.new(0.0, 0, 0.28, 0),
					Parent = label,
					ZIndex = 8,
				},
				Scaled = true,
		...
		if gui.main then
			...
			for k, v in pairs(btnData) do
				local btn = main[k]
				btn.txt:Destroy()
				for i, v in pairs({"txt2", "FighterIcon"}) do
		...
		write 'Cancel' {
			Frame = create 'Frame' {
				...
				BackgroundTransparency = 1.0,
				Size = UDim2.new(1.0, 0, 0.42, 0),
				Position = UDim2.new(0.0, 0, 0.28, 0),
				Parent = cancel,
			},
			Scaled = true,
	...
	function BattleGui:chooseMoveTarget(moveNum, move, rqPokemon, userPosition, nActive)
		...
		if gui.zmove.selected then
			pcall(function()
				moveTarget = rqPokemon.canZMove[moveNum].target or moveTarget
			end)
		elseif gui.dynamax.selected or gui.gigantamax.selected or rqPokemon.currentDyna then
			pcall(function()
				moveTarget = rqPokemon.maxMoves[moveNum].target or moveTarget
			end)
		end
		...
		self.onTargetClicked = function(n)
			...
			if getTokenForPosition(n) > 0 then
				...
				if gui.mega and gui.mega.selected then option = ' mega ' elseif gui.zmove and gui.zmove.selected then option = ' zmov ' elseif gui.ultra and gui.ultra.selected then option = ' ultra ' elseif ((gui.dynamax and gui.dynamax.selected) or (gui.gigantamax and gui.gigantamax.selected)) then option = ' dynamax ' elseif rqPokemon.currentDyna then option = ' maxmove ' end
		...
		local fight, ms, zm, dm, gm = main.fight, gui.mega.spriteLabel, gui.zmove.spriteLabel, gui.dynamax.spriteLabel, gui.gigantamax.spriteLabel
		...
		Utilities.Tween(.6, 'easeOutCubic', function(a)
			...
			zm.Position = ms.Position
			dm.Position = ms.Position
			gm.Position = ms.Position
	...
	function BattleGui:fightChoices(moveset, rqPokemon, slot, nActive, alreadyChoseMega)
		local validZMoves = {}
		...
		self.onMoveClicked = function(m)
			...
			if not gui.zmove.selected then
				if move.pp <= 0 then
					state = 'canteven'
				...
				if not validZMoves[m] then return end
			end
			...
			if nActive > 1 then
				...
				local option = ''
				if gui.mega.selected then
					option = ' mega'
				elseif gui.zmove.selected then
					option = ' zmov'
				elseif (gui.dynamax.selected or gui.gigantamax.selected) then
					option = ' dynamax'
				elseif rqPokemon.currentDyna then
					option = ' maxmove'
				end
		...
		local main = gui.main
		local container = main.container
		local moves, mega, cancel, zmove, dynamax, gigantamax = gui.moves, gui.mega, gui.cancel, gui.zmove, gui.dynamax, gui.gigantamax

		if not moves then
			moves = {false, false, false, false}
			for i = 1, 4 do
				moves[i] = create 'ImageLabel' {
					Name = 'Move'..i,
					BackgroundTransparency = 1.0,
					Image = 'rbxassetid://7018618985',
					Size = UDim2.new(1.0, 0, 1.0, 0),
					...
					create 'Frame' {
						Name = 'TypeContainer',
						BackgroundTransparency = 1.0,
						Size = UDim2.new(0.0, 0, 0.23, 0),
						Position = UDim2.new(0.4, 0, 0.595, 0),
						ZIndex = 8,
					},
					create 'Frame' {
						Name = 'PPContainer',
			...
			s.MouseButton1Click:connect(function()
				if dynamax.paused then
					...
					if gui.gigantamax.selected then
						gui.gigantamax.selected = false
						gui.gigantamax:Pause()
					end
					dynamax:Play()
		...
		local zMoves = rqPokemon and rqPokemon.canZMove
		local maxMoves = rqPokemon and rqPokemon.maxMoves
		...
		function self.moveUpdate(og)
			for i = 1, 4 do
				if gui.zmove.selected then
					local z = zMoves and zMoves[i] ~= 0 and zMoves[i] or nil
					validZMoves[i] = self:updateButtonForZMove(moves[i], moveset[i], z, zMoves)
				elseif gui.dynamax.selected or gui.gigantamax.selected or rqPokemon.currentDyna then
					self:updateButtonForMaxMove(moves[i], moveset[i], maxMoves and maxMoves[i])
				else
					validZMoves = {}
					self:updateButtonForMove(moves[i], moveset[i], og)
				end
		...
		if rqPokemon and rqPokemon.currentDyna then
			self.maxMoveUpdate()
		else
			self.moveUpdate()
		end
		...
		if rqPokemon and rqPokemon.canDynamax then
			dynamax.spriteLabel.Visible = true
		else
			dynamax.spriteLabel.Visible = false
		end
		if rqPokemon and rqPokemon.gigantamax then
			gigantamax.spriteLabel.Visible = true
		else
			gigantamax.spriteLabel.Visible = false
		end

		local ms, zs, dm, gm = mega.spriteLabel, zmove.spriteLabel, dynamax.spriteLabel, gigantamax.spriteLabel
		Utilities.Tween(.6, 'easeOutCubic', function(a)
			local o = 1-a
			run.Position = UDim2.new(0.0, 0, 135/130/2, 0) + UDim2.new(0.0, 0, 0.0, (Utilities.gui.AbsoluteSize.Y-container.AbsolutePosition.Y+run.AbsoluteSize.Y)*a)
			bag.Position = UDim2.new(-424/522, 0, 0.0, 0) + UDim2.new(0.0, -(container.AbsolutePosition.X+bag.AbsoluteSize.X)*a, 0.0, 0)
	...
	function BattleGui:exitButtonsMoveChosen()
		state = 'animating'
		local main = gui.main
		local fight, ms, zs, dm, gm = main.fight, gui.mega.spriteLabel, gui.zmove.spriteLabel, gui.dynamax.spriteLabel, gui.gigantamax.spriteLabel
		local container = main.container
	...
	function BattleGui:animGMax(battle, poke)
		...
		if sideNumber == 1 then
			...
			sideDirection = 1
			do
				local localSpriteLabel = sprite.animation.spriteLabel
				local localSpriteSize = localSpriteLabel.ImageRectSize
				localSpriteLabel.ImageRectSize = Vector2.new(-localSpriteSize.X, localSpriteSize.Y)
				function sprite.animation.updateCallback()
					local spriteOffset = localSpriteLabel.ImageRectOffset
					localSpriteLabel.ImageRectOffset = Vector2.new(spriteOffset.X + localSpriteSize.X, spriteOffset.Y)
				end
			end
		end
		...
		local cp = gmaxField.PlayerPos.Position + Vector3.new(0, gmaxField.PlayerPos.Size.Y / 2 + 1.8, 0)
		...
		local function doSeparateThreadTween(obj, prop, val, dur, timing)
			task.spawn(function()
				task.wait()
				Utilities.pTween(obj, prop, val, dur, timing)
			end)
		end

		doSeparateThreadTween(cam, 'FieldOfView', oldFov, 1, 'easeInCubic')
		Utilities.pTween(cam, 'CFrame', gmaxField.HandZoom.CFrame, 1, 'easeInCubic')
		gmaxBall.Parent = workspace
		ballAppear(gmaxBall)
		doSeparateThreadTween(cam, 'FieldOfView', oldFov + 10, 1, 'easeInOutCubic')
		...
		if trainer then
			...
			task.delay(holdDur, function()
				doSeparateThreadTween(cam, 'FieldOfView', 30, 1, 'easeInOutCubic')
	...
	function BattleGui:cancelToMain()
		...
		local fight, bag, pokemon, run = main.fight, main.bag, main.pokemon, main.run
		local container = main.container
		local moves, ms, cancel, zs, dm, gm = gui.moves, gui.mega.spriteLabel, gui.cancel, gui.zmove.spriteLabel, gui.dynamax.spriteLabel, gui.gigantamax.spriteLabel
	...
	function BattleGui:switchPokemon(forced, chooseSlot, alreadySwitched, toSlot)
		...
		if battle and battle.isRaid then
			self:message("You can't switch Pokemon during raids!")
			return false
		else
			spawn(function()
				-- ugly hack, BUT much less ugly with PDS up-to-date-with-battle party data
	...
	function BattleGui:createFoeHealthGui(nActive, slot)
		...
		hg.main = create 'Frame' {--create 'ImageLabel' { --roundedFrame:new {
			...
			create 'ImageLabel' {
				Name = 'OwnedIcon',
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://7824188301', --PBallIcon
				SizeConstraint = Enum.SizeConstraint.RelativeYY,
		...
		hg.typer = roundedFrame:new {
			...
			create 'Frame' {
				Name = 'text',
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, 0.63, 0),--1.0, -hpg*2),
				Position = UDim2.new(0.5, 0, 0.12, 0),--0.0, hpg),
				ZIndex = 5,
			},
		}
		hg.typel = roundedFrame:new {
			Name = 'type1',
			Size = UDim2.new(0.25, 0, 0.35, 0),
			Position = UDim2.new(0.325, 0, 1, 0),
			Style = 'HorizontalBar',
			ZIndex = 4,
			Parent = hg.main,--.gui,
		...
		function hg:update(pokemon)
			...
			-- types
			local typeFromInt = {'Bug','Dark','Dragon','Electric','Fairy','Fighting','Fire','Flying','Ghost','Grass','Ground','Ice','Normal','Poison','Psychic','Rock','Steel','Water','Admin'}
			local types = {}
			local left, right = self.typel, self.typer

			for i, n in pokemon.types do types[i] = typeFromInt[n]    end

			local type1, type2 = types[1], types[2]
			local c1 = typeColors[type1]

			if type2 then
				local c2 = typeColors[type2]
				left.Visible = true
				left.BackgroundColor3 = c1
				right.BackgroundColor3 = c2
				...
				write(type2){
					Frame = right.gui.text,
					Scaled = true
				}
			else
				left.Visible = false
				right.BackgroundColor3 = c1
		...
		function hg:destroy()
			...
			pcall(function() self.typer:Destroy() self.typel:Destroy() end)
			self.main:Destroy()
		end
		return hg
	end
	...
	function BattleGui:createUserHealthGui(nActive, slot)
		...
		hg.main = create 'Frame' {--'ImageLabel' {--roundedFrame:new {
			...
			create 'Frame' {
				Name = 'NameContainer',
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, (0.35/ys-0.02), 0),
				Position = UDim2.new(0.06, 0, 0.125/ys, 0),
			...
			create 'Frame' {
				Name = 'LevelContainer',
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, (0.25/ys-0.014), 0),
				Position = UDim2.new(0.75, 0, (0.22/ys+0.002), 0),
			...
			create 'Frame' {
				...
				BackgroundTransparency = 1.0,
				Size = UDim2.new(0.0, 0, (0.25/ys-0.008), 0),
	...
	function BattleGui:promptReplaceMove(movesData, alreadyFaded)
		...
		local function guiForMove(move, onClick)
			...
			local panel = roundedFrame:new {
				...
				create 'ImageLabel' {
					BackgroundTransparency = 1.0, -- Status, Special, & Physical Decals
					Image = 'rbxassetid://'..({Status=9603427836,Special=9603427752,Physical=9603427643})[move.category],
	...
	function BattleGui:animateEvolution(name1, name2, sd1, sd2, alreadyFaded, cannotBeCanceled)
		...
		delay(1, function()
			...
			sound = Utilities.loopSound(Audios.EvolutionTheme1)
		end)
		...
		spawn(function()
			...
			for i = 1, n do
				...
				local image = Utilities.Create 'ImageLabel' {
					BackgroundTransparency = 1.0,
					Image = 'rbxassetid://289114252', -- Smack 1
		...
		local image = create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://13044889338', -- Smack 1
