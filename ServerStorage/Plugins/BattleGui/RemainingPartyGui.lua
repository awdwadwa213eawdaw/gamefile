return function(_p)
	...
	local rPartyThread
	local status_normal = 'rbxassetid://7824188301' -- PBallIcon
	local status_faint  = 'rbxassetid://140462613085888' -- PBallSilent
	return function(self, on)
		...
		if not left and not battle.isSafari then
			left = create 'Frame' {
		...
		local xsl, xel
		if not battle.isSafari then
			xsl = left.Position.X.Offset
			xel = on and math.abs(left.AbsoluteSize.X) or -math.abs(left.Parent.AbsoluteSize.X)*0.05
		end
