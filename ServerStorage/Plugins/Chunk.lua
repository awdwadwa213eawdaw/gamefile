return function(_p)
	...
	local compareMusic = function(music1, music2)
		if type(music1) == "table" then
			if type(music2) ~= "table" or #music1 ~= #music2 then
				return false
			end
			for i = 1, #music1 do
				if music1[i] ~= music2[i] then
					return false
				end
			end
			return true
	...
	function Chunk:startRegionMusic()
		local regionData = self.regionData
		if regionData and regionData.Music then
			local v29 = _p.MusicManager:stackMusic(regionData.Music, "RegionMusic", 3, regionData.MusicVolume)
			if regionData.MusicFilterBass then
				v29.SoundGroup = _p.WalkEvents.ReduceBass
			end
			if self.StartMusicAtZeroVolume then
				v29.Volume = 0
				local u9 = nil
				u9 = v29.Changed:connect(function()
					if not self.StartMusicAtZeroVolume then
						u9:disconnect()
						return
					end
					if v29.Volume > 0 then
						v29.Volume = 0
					end
				end)
			end
	...
	function Chunk:newSign(newRegion)
		local thread = {}
	...
	function Chunk:changedRegions(newRegion)
		...
		local thread = {}
		self.regionThread = thread
		local board = create 'ImageLabel' { -- 400x150
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://6222644505',
			SizeConstraint = Enum.SizeConstraint.RelativeYY,
			Size = UDim2.new(0.6, 0, 0.225, 0),
		...
		local text = create 'Frame' {
			...
			ZIndex = 2, Parent = board.Container,
		}
		local written = Utilities.Write(newRegion.Name) {
			Frame = text,
			Scaled = true, TextXAlignment = Enum.TextXAlignment.Left,
		}
		spawn(function()
			local limit = written.MaxBounds.X --[[/ written.MaxBounds.Y * board.Container.AbsoluteSize.Y]] + board.Container.AbsoluteSize.X--*0.8
			local start = tick()
			local rate = 3
			while true do
				if self.regionThread ~= thread then break end
				local et = tick()-start
				local p = et*rate*text.AbsoluteSize.Y
				text.Position = UDim2.new(1.0, -p, 0.2, 0)
				if p > limit then break end
				stepped:wait()
			end
			Utilities.Tween(.5, 'easeOutCubic', function(a)
				if self.regionThread ~= thread then return false end
	...
	function Chunk:getCaveDoor(id)
		if next(self.caveDoors) ~= nil then
			for _, doorPart in pairs(self.caveDoors) do
				local otherChunk = doorPart.Name:match('^CaveDoor:([^:|]+):?([^|]*)|?(.*)$')
				if otherChunk == id then
					return doorPart
				end
			end
		end
	...
	function Chunk:getRoom(id, door, level)
		...
		local weather = _p.Overworld.currentWeather	
		if weather ~= 'spacial' then	
			_p.Overworld:toggleWeatherIndoors(false)
		end
		--// Disable Weather Indoors
	...
	function Chunk:getRoomMusic(roomId)
		...
		if roomId == 'PokeCenter' then
			roomMusicId = 88765108726835
		elseif roomId:sub(1, 4) == 'Gate' and tonumber(roomId:sub(5)) then
			roomMusicId = 131129812202603--288893317
			roomMusicVolume = .45
		end
		pcall(function()
	...
	function Chunk:registerEnterDoorEvent(name, func)
		self.enterDoorEvents[name] = func
	end
	function Chunk:registerExitDoorEvent(name, func)
		self.exitDoorEvents[name] = func
	end
	...
	function Chunk:enterDoor(door)
		...
		if not (table.find({"C_chunk96|a", "C_chunk96|b"}, door.id) and self.id == "chunk97") then 
		...
		if door.id:sub(1, 2) == 'C_' then -- door leads to another chunk instead of room
			...
			if door.id == "C_chunkMewtwo" then
				walkFrom = door.CFrame * CFrame.new(0, -door.Size.y/2+3, 2) * CFrame.Angles(0, math.rad(180), 0)
			end
	...
	function Chunk:exitDoor(door, alreadyFaded)
		...
		local weather = _p.Overworld.currentWeather	
		if weather ~= 'spacial' then	
			_p.Overworld:toggleWeatherIndoors(true)
		end
	...
	function Chunk:hookupCaveDoor(doorPart, roomId)
		...
		table.insert(self.caveDoors, doorPart)
		...
		doorPart.Touched:connect(function(p)
			...
			if self.doorDebounce or doorPart.Locked then return end
			_p.Hoverboard:unequip()
			self.doorDebounce = true
			MasterControl.WalkEnabled = false
			MasterControl:Stop()
			if otherChunkId ~= "gym8" then
			...
			local uglyFixCF = CFrame.new(2817, 1260.843, -4643.582) * CFrame.fromOrientation(math.rad(-20), math.rad(180), 0)
			if thisChunkId == "chunkVC4" and otherChunkId == 'chunkE4' then
				local currentCf = cam.CoordinateFrame
				spawn(function() Utilities.Tween(.5, 'easeOutCubic', function(a) cam.CoordinateFrame = currentCf:Lerp(uglyFixCF, a) end) end)
			else
			...
			if newChunk.id == "gym8" then
				local Gym8Spawn = newChunk.map.Room3.GymSpawn.CFrame
				Utilities.fadeGui.BackgroundColor3 = Color3.new(0, 0, 0)
				Utilities.fadeGui.BackgroundTransparency = 0
				circ[1]:Destroy()
				_p.DataManager.chunkTransitionFadeCircle = nil
				_p.Surf:forceUnsurf()
				Utilities.Teleport(Gym8Spawn)
				cam.CameraType = Enum.CameraType.Custom
				task.wait()
				cam.CFrame = CFrame.new(cam.Focus.p + Vector3.new(0, 4, -8).unit * 12.5, cam.Focus.p)
				cam.CameraType = Enum.CameraType.Scriptable
				Utilities.FadeIn(1)
				cam.CameraType = Enum.CameraType.Custom
				newChunk.doorDebounce = false
			...
			local s, r = pcall(function() doorCF = doorPart.CFrame end)
			if not s then
				error("Part is not positioned properly for "..tostring(doorPart.Name))
				return
			end
			pos = doorCF.p
			...
			local pcf
			local focus
			if newChunk.id == 'chunk54' and thisChunkId == 'gym8' then
				pcf = doorCF + Vector3.new(8,-5,-6)
				focus = pcf.p
			else
				pcf = doorCF * CFrame.new(0, -doorPart.Size.Y/2+3, 0)
				focus = pcf.p
			...
			local event = _p.Events['onCaveDoorFocused_'..doorPart.Name..'|'..newChunk.id] or _p.Events['onCaveDoorFocused_'..newChunk.id]
	...
	function Chunk:hookupItem(itemModel)
		...
		part.Touched:connect(function(p)
			...
			Utilities.sound(122067899999821, nil, nil, 10)
	...
	function Chunk:init(withData) -- to be called once the chunk is actually in the workspace
		...
		for _, obj in pairs(self.map:GetChildren()) do
			...
			elseif obj.Name == '#Item' then
				self:hookupItem(obj)
			elseif not ignoreNPCs and obj:IsA'Model' and obj:FindFirstChild'Humanoid' and obj.Name ~= "FraudbernFire" then
