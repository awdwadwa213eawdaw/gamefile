return function(_p)--local _p = require(script.Parent)
				...
				if _p.BitBuffer.GetBit(_p.PlayerData.defeatedTrainers, v)then
					if battle.RematchQuestion then
						_p.NPCChat.interactableNPCs[model] = function()
							...
							if fight then
								...
								if win and not battle.RematchQuestion then
									_p.NPCChat.interactableNPCs[model] = battle.Interact
								end
							end
						end
					elseif battle.Interact and not battle.RematchQuestion then
						_p.NPCChat.interactableNPCs[model] = battle.Interact
					end
				else
					self.battle = battle
					...
					do
						...
						if model:FindFirstChild("IsSwimmer") then
							self.Swimmer = true
						elseif --[[chunk.id == 'chunk5' and]] model.Parent.Name == 'Gym2' then
							self.gym2 = true
						end
					end
					...
					_p.NPCChat.interactableNPCs[model] = function()
						...
						if win then
							...
							if not battle.RematchQuestion then
								_p.NPCChat.interactableNPCs[model] = battle.Interact
							end
						else
	...
	function NPC:collectNPCs(model, collection) -- old API, didn't use much
		while true do
			...
			tag.Parent:Destroy()
			local m = _p.storage.Models.NPCs[name]:Clone()
	...
	function NPC.trainersCheck()
		...
		local p = _p.player.Character:WaitForChild("HumanoidRootPart").Position
		for _, t in pairs(watchingTrainers) do
			if t.model and t.model:FindFirstChild('HumanoidRootPart') then
				...
				if t.Swimmer and _p.Surf.surfing then
					if cf.p.y - 1 < p.y then
						yCondition = p.y < cf.p.y + 15
					end
					mag = 180
				elseif t.gym2 then
					yCondition = cf.p.y-1 < p.y and cf.p.y+15 > p.y
				end
				if yCondition and (cf.p-p).magnitude < mag then
					local theta = angle(cf, Vector3.new(p.x, cf.p.y, p.z))
					if math.deg(math.abs(theta)) < 15 then
						if t.model:FindFirstChild('CanBattle') then
							if t.model:FindFirstChild('CanBattle').Value == false then
								return 
							end
						end
						...
						if t.battle.IgnoreWalls or (Utilities.findPartOnRayWithIgnoreFunction(Ray.new(thead.Position, (phead.Position-thead.Position)), function(p) return p~=phead and (not p.CanCollide or p.Parent==character or p.Parent:FindFirstChild('Humanoid')) end)) == phead then
							...
							if win then
								...
								if t.battle.Interact and not t.battle.RematchQuestion then
	...
	function NPC:Animate()
		...
		pcall(function() model.Humanoid:Destroy() end)
		model:BreakJoints()
		...
		if self.model:FindFirstChild("IsSwimmer") then
			--[[Idle Anim]] self.humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.NPCIdleSwim }):Play()
			...
			--[[Idle Anim]] self.humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.NPCIdle }):Play()
	...
	function NPC:Rig()
		...
		local torso = model.Torso -- consideration: will we ever have R15 NPCs?
	...
	function NPC:SetInteract(...)
		pcall(function() self.model.Interact:Destroy() end)
		create 'StringValue' {
			Name = 'Interact',
	...
	function NPC:Destroy() self:destroy() end
	function NPC:destroy()
		self.walkThread = nil
		pcall(function() self.model:Destroy() end)
		self.model = nil
		pcall(function() self.animate:Destroy() end)
		self.animate = nil
