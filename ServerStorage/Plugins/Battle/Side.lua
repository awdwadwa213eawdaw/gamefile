return function(_p)
	...
	local Pokemon = require(script.Parent.Pokemon)
	...
	local Side = class({
		...
		self.pokemon = {}
		self.pokemonLeft = self.totalPokemon
	...
	function Side:newPokemon(species, replaceSlot)
		...
		if pokeobj then
			poke = extend(poke, pokeobj)
		end
		if poke.ability == '' and poke.baseAbility ~= '' then
	...
	function Side:getTeamName()
		if self == self.battle.mySide then
			return 'Your team'
		end
	...
	function Side:switchOut(pokemon, slot)
		...
		if pokemon.statbar then
			pokemon.statbar:slideOffscreen(true)
			pokemon.statbar = nil
		end
	end
		...
		if not silent then
			if pokemon.side.n == 1 then
				if self.battle.skipGoFlag then
					...
					msgFn = function()
						self.battle:message('Go! ' .. pokemon:getShortName() .. '!')
					end
				end
			elseif self.battle.kind == 'wild' then
				-- wild battle message handled elsewhere
			else
				msgFn = function()
					self.battle:message(pokemon.side:getNameForSlot(slot) .. ' sent out ' .. pokemon:getShortName() .. '!')
				end
			end
		end

		if not (pokemon.side.n == 1 and self.battle.isSafari and self.battle.kind == 'wild') then
	...
	function Side:swapTo(pokemon, slot, kwargs)
		...
		if pokemon:hasVolatile('substitute') then
			pokemon.sprite:animSubFade()
		end
		if target and target:hasVolatile('substitute') then
			target.sprite:animSubFade()
		end

		spawn(function()
			pokemon.statbar:slideOffscreen(false)
		end)
		if target then
			spawn(function()
				target.statbar:slideOffscreen(false)
			end)
		end
		...
		spawn(function()
			pokemon.statbar:evalYPos(true)
			pokemon.statbar:slideOnscreen()
		end)
		if target then
			spawn(function()
				target.statbar:evalYPos(true)
				target.statbar:slideOnscreen()
			end)
		end
	...
	function Side:faint(pokemon, slot)
		...
		spawn(function()
			self.battle:message(pokemon:getName() .. ' fainted!')
		end)
		...
		self.pokemonLeft = (self.pokemonLeft or self.totalPokemon or 6) - 1

		if self.battle.faintCallback then
			self.battle:faintCallback(self)
		end
	end
