...
return function(Battle, _p)
	...
	function Battle:useMove(pokemon, move, target, kwargs, zMove)
		...
		local RemoveZMoveName
		if not kwargs.silent then
			local id = fromeffect.id
			if move.isZ then
				self:message(pokemon:getName() .. " unleashes its full-force Z-Move!")
				RemoveZMoveName = self:showZMoveName({
					move.name, 
					move.type
				})
				self:stopZPowerGlow(pokemon.sprite)			
			end
			if id == 'snatch' or id == 'toxicdebris' then
	...
	function Battle:useMultihitMove(pokemon, move, target, kwargs, actions, zMove)
		...
		for i, a in pairs(actions) do
			--print("Action ", i, ": ", a)
			if a:sub(1, 8) == '|-damage' and not a:find("%[of%]") then -- damages are reversed
		...
		while #minors > 0 do
			...
			elseif arg1 == '-damage' then
				...
				if damage then -- ~= false then --
					...
					elseif kwargs.from then
						...
						elseif effectId == 'frz' then
							addAction(poke:getName() .. " was hurt by frostbite!")
			...
			elseif arg1 == '-boost' then
				...
				elseif kwargs.from then
					...
					elseif effect.name == "Commanded" then
						addAction(poke:getName() .. "'s commanding ally" .. amountString .. " raised its " .. BattleStats[stat] .. "!")
					elseif not string.find(effect.name, "maxraid") then
			...
			elseif arg1 == '-unboost' then
				...
				elseif kwargs.from then
					...
					elseif effect.name == "Mirror Armor" then
						addAction(poke:getName() .. " had its " .. BattleStats[stat] .. " " .. amountString .. "lowered by " .. effect.name .. "!")
					elseif effect.name == "Supersweet Syrup" then
			...
			elseif arg1 == '-copyboost' then
				...
				local stats = args[4] and split(args[4], ', ') or {'atk', 'def', 'spa', 'spd', 'spe', 'accuracy', 'evasion'}
				local effect = Tools.getEffect(kwargs.from)
				local copyVol = {'focusenergy', 'gmaxchistrike', 'laserfocus'}
				for i, stat in pairs(stats) do
					poke.boosts[stat] = frompoke.boosts[stat]
				end
				for i, vol in pairs(copyVol) do
					if frompoke.volatiles[vol] then
						poke:addVolatile(vol)
					else
						poke:removeVolatile(vol)
					end
				end
				...
				if kwargs.silent then
					...
					addAction(poke:getName() .. " copied " .. frompoke:getLowerName() .. "'s stat boosts!")
				end
			...
			elseif arg1 == '-zbroken' then
				addAction(self:getPokemon(args[2]):getName()..' couldn\'t fully protect itself and got hurt!')
				self:getPokemon(args[2]):removeTurnstatus('protect')

				for _, p in pairs(self:getPokemon(args[2]).side.pokemon) do
					p:removeTurnstatus('wideguard')
					p:removeTurnstatus('quickguard')
					p:removeTurnstatus('craftyshield')
					p:removeTurnstatus('matblock')
					self:getPokemon(args[2]).side:updateStatbar(p)
				end
			elseif arg1 == '-boostFromZEffect' then
				local amountString = ''
				local amount = tonumber(args[4])
				self:animBoost(self:getPokemon(args[2]))				
				if amount == 2 then amountString = ' sharply'
				elseif amount >= 3 then amountString = ' drastically' end
				addAction(self:getPokemon(args[2]):getName()..' boosted its '..BattleStats[args[3]]..''..amountString..' using its Z-Power!')
			elseif arg1 == '-boostMultipleFromZEffect' then
				self:animBoost(self:getPokemon(args[2]))				
				addAction(self:getPokemon(args[2]):getName()..' boosted its stats using its Z-Power!')
			elseif arg1 == '-healFromZEffect' then
				addAction(self:getPokemon(args[2]):getName()..' restored its HP using its Z-Power!')
			elseif arg1 == '-healreplacement' then
				addAction(self:getPokemon(args[2]):getName()..' will restore its replacement\'s HP using its Z-Power!')
			elseif arg1 == '-clearnegativeboost' then
				local poke = self:getPokemon(args[2])
				addAction(poke:getName() .. "'s negative stats where cleared!")
			elseif arg1 == '-clearpositiveboost' then
				local poke = self:getPokemon(args[2])
				local fromPoke = self:getPokemon(args[3])
				local effect = Tools.getEffect(args[4])
				for _, stat in pairs(poke.boosts) do
					if (poke.boosts[stat] and poke.boosts[stat] > 0) then
						poke.boosts[stat] = 0
					end
					--self:resultAnim(poke, 'Boosts lost', 'bad') todo
				end
				if (effect and effect.id and effect.id == 'spectralthief') then
					self:animBoost(fromPoke)
					addAction(fromPoke:getName() .. " has stolen their opponent's boosts!")
					--self:runOtherAnim('spectralthiefboost', {fromPoke, poke}, self) todo
				end
			...
			elseif arg1 == '-immune' then
				...
				elseif effect.id == 'commanding' then
					addAction(poke:getName() .. " is protected in its partner's mouth!")
				else
					if kwargs.msg then
			...
			elseif arg1 == '-enditem' then
				...
				elseif effect.id and item.id ~= 'throatspray' and item.id ~= 'boosterenergy' and item.id ~= 'ejectpack' then
					...
					elseif item.id == 'throatspray' or item.id == 'boosterenergy' then
						--Do nothing
					else
			...
			elseif arg1 == '-ability' then
				...
				elseif effect.id == 'lingeringaroma' then
					addAction(poke:getName() .. "'s Ability became Lingering Aroma!")
				...
				elseif effect.id and effect.id ~= '' then
					...
					elseif ability.id == 'electromorphosis' then
						addAction(poke:getName() .. " is charging power!")
					...
					elseif ability.id == 'neutralizinggas' then
						addAction("All other abilities have been neutralized by " .. poke:getName() .. "!")
			...
			elseif arg1 == '-endability' then
				...
				elseif ability and ability.id == 'illusion' then
					...
					pcall(function() poke.sprite:renderNewSpriteData(nil, true) end)
					pcall(function() poke.statbar:update() end)
			...
			elseif arg1 == '-formechange' then
				...
				if kwargs.silent then
					...
					elseif template.species == 'Darmanitan' then
						...
						elseif fromeffect.name == 'Zen Mode' then
				...
				if not brk then
					...
					poke.types = template.types or poke.types
			...
			elseif arg1 == '-mega' then
				...
				if args[3] == 'Necrozma' then
					self:message('Bright light is about to burst out of '..poke:getName().."!")
				elseif args[3] == 'Rayquaza' then
			...
			elseif arg1 == '-zmove' then
				...
				local move = args[3]
				task.wait(0.5)
				pokemon.sprite:animZDance(pokemon, self,storage.Models.Rpoke.Value)
			elseif arg1 == '-start' then
				...
				if effect and effect.id ~= "typechange" then
					poke:addVolatile(effect.id)
				end

				if kwargs.silent and (effect and effect.id and effect.id ~= 'typechange' and effect.id ~= 'typeadd' or not effect.id) then
					-- do nothing
				elseif effect and effect.id == 'typechange' then
				...
				elseif effect and effect.id == 'typeadd' then
					--					poke.volatiles.typeadd[3] = args[4]
					if not kwargs.silent then
						addAction(args[4] .. " type was added to " .. poke:getLowerName() .. "!")
					end
				elseif effect and effect.id == 'powertrick' then
					addAction(poke:getName() .. " switched its Attack and Defense!")
				elseif effect and effect.id == 'foresight' or effect.id == 'miracleeye' then
					addAction(poke:getName() .. " was identified!")
				elseif effect and effect.id == 'telekinesis' then
					addAction(poke:getName() .. " was hurled into the air!")
				elseif effect and effect.id == 'confusion' then
				...
				elseif effect and effect.id == 'leechseed' then
					poke.side:updateStatbar(poke)
					addAction(poke:getName() .. ' was seeded!')
				elseif effect and effect.id == 'protosynthesis' then
					self:animBoost(poke)
					addAction(poke:getName() .. "'s " .. BattleStats[args[4]] .. " was raised by Protosynthesis!")
				elseif effect and effect.id == 'quarkdrive' then
					self:animBoost(poke)
					addAction(poke:getName() .. "'s " .. BattleStats[args[4]] .. " was raised by Quark Drive!")	
				elseif effect and effect.id == 'healblock' then
				...
				elseif effect.id == 'dynamax' then
				elseif effect.id == 'maxraid' then
				...
				elseif effect.id == 'powder' then
					...
					addAction(poke:getName() .. "'s " .. tostring(effect.name) .. " started!")
				end
				poke.side:updateStatbar()
			elseif arg1 == '-end' then
				...
				if args[3] == 'Dynamax' then
					poke.sprite.isDyna = false

					self:animStatus('redmax', poke)
					if storage.Models.Max.Value == 'Gmax' then
						poke.sprite:Animgmax(false)
					end	
					self:stopZPowerGlow(poke.sprite)			
				end
				if kwargs.silent or not effect.id then
					-- do nothing
				...
				elseif ({bind=true,wrap=true,clamp=true,whirlpool=true,firespin=true,magmastorm=true,sandtomb=true,infestation=true})[effect.id] then
					...
					if effect.effectType == 'Move' then
						...
						addAction(poke:getName() .. "'s " .. tostring(effect.name) .. " ended!")
					end
				end
				poke.side:updateStatbar()
			elseif arg1 == '-singleturn' then
				...
				elseif effect.id == 'maxguard' then
					addAction(poke:getName() .. ' guarded itself!')
			...
			elseif arg1 == '-activate' then
				...
				elseif effect.id == 'poltergeist' then
					addAction(poke:getName() .. ' was attacked by its ' .. args[4] .. "!")
				elseif effect.id == 'snatch' then
				...
				elseif effect.id == 'wanderingspirit' then
					self:showAbility(poke, 'Wandering Spirit')
					local ability = Tools.getAbility(args[4])
					task.wait(.4)
					self:showAbility(ofpoke, ability.name)
					task.wait(1)
					self:showAbility(ofpoke, 'Wandering Spirit')
					task.wait(.4)
					self:showAbility(poke, ability.name)
					addAction(ofpoke:getName() .. " swapped abilities with ".. poke:getName() .."!")
				elseif effect.id == 'lingeringaroma' then
					self:showAbility(poke, 'Lingering Aroma')
					local ability = Tools.getAbility(args[4])
					task.wait(.4)
					self:showAbility(ofpoke, ability.name)
					task.wait(1)
					self:showAbility(ofpoke, 'Lingering Aroma')
					addAction(ofpoke:getName() .. "'s Ability became Lingering Aroma!")
				...
				elseif effect.id == 'powerconstruct' then
					self:message('Zygarde is now transforming into its Complete form!')
					self:showAbility(poke, 'Power Construct')
				elseif effect.id == 'terashift' then
					self:message('Terapagos is now transforming into its Terastal form!')
					self:showAbility(poke, 'Tera Shift')
			...
			elseif arg1 == '-sidestart' then
				...
				side:addSideCondition(effect, duration)
			...
			elseif arg1 == '-swapsideconditions' then
				self:swapSideConditions()
			elseif arg1 == '-weather' then
				...
				if args[3] then
					self:changeWeather(toId(args[2]), poke, effect, kwargs.upkeep, args[3])
				else
					self:changeWeather(toId(args[2]), poke, effect, kwargs.upkeep)
				end
			elseif arg1 == '-weatherstart' then
				self.weatherTimeLeft = args[2]
			elseif arg1 == '-terrainstart' then
				self.terrainTimeLeft = args[2]
			elseif arg1 == '-raidenraged' then
				local poke = self:getPokemon(args[2])
				self:message(poke:getName() .. " has become enraged for 5 turns!")
			elseif arg1 == '-raidshield' then
				local poke = self:getPokemon(args[2])
				self:message(poke:getName() .. " has put up their shield for 2 turns!")
			elseif arg1 == '-terrain' then
				--self:message(args[2] .. " has " .. args[3])
				self.terrainTimeLeft -= 1
			elseif arg1 == '-fieldstart' then
				...
				local duration = 5
				if kwargs.pesistent then
					duration += 2
				end
				if endsWith(effect.id, "terrain") then
					self:changeTerrain(effect.id)
				else
					self:addPseudoWeather(effect.name, duration)
				end

				if fromeffect and fromeffect.effectType == 'Ability' then
					self:showAbility(poke, fromeffect.name)
				end
			...
			elseif arg1 == '-lvlup' then
				...
				self:sound(88976115632409, nil, nil, 5)
			...
			elseif arg1 == '-capture' then
				...
				local sbCount = tonumber(args[5])
				--print(poke, ballId, shakes, kwargs.crit and true or false, sbCount)
				self:animCapture(poke, ballId, shakes, kwargs.crit and true or false, sbCount)
				-- elseif nickname?
			...
			elseif arg1 == '-animBerryThrow' then
				local rate, berryName, berryColor = tonumber(args[3]), args[4], args[5]				
				local poke = self:getPokemon(args[2])
				self:animBerryThrow(rate, poke, berryName, berryColor)
	...
	function Battle:runMajor(args, kwargs)
		...
		elseif arg1 == 'detailschange' then
			...
			elseif kwargs.battleBond then
				...
				self:message(poke:getName() .. " transformed into its Ash Greninja!")
			elseif kwargs.zeroToHero then --ZtH also uses Mega Evolution/Ash Greninja Animation
				poke.forme = 'Hero'
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
			...
			elseif kwargs.powerConstruct then --Power Construct
				poke.forme = 'Complete'
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
				local spriteId = (poke.shiny and '_SHINY' or '')..(poke.side.n == 1 and '_BACK/' or '_FRONT/')..'Zygarde-Complete'
				poke.sprite:animMegaEvolve(Tools.getSprite(spriteId), 'Black metallic', 'Bright green', 'Dark green')
				self:message(poke:getName() .. " transformed into its Complete form!")
			elseif kwargs.teraShift then
				poke.forme = 'Terastal'
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
				local spriteId = (poke.shiny and '_SHINY' or '')..(poke.side.n == 1 and '_BACK/' or '_FRONT/')..'Terapagos-Terastal'
				poke.sprite:animMegaEvolve(Tools.getSprite(spriteId), 'Deep Blue', 'Cyan', 'Teal')
				self:message(poke:getName() .. " transformed into its Terastal form!")
				...
				--				poke.side:updateStatbar()
				-- todo
			elseif kwargs.flowerGift then
				poke.forme = 'Sunshine'
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
				poke.baseStatOverride = Tools.getTemplate(templateId).baseStats
				local spriteId = (poke.shiny and '_SHINY' or '')..(poke.side.n == 1 and '_BACK/' or '_FRONT/')..'Cherrim-Shunshine'
			elseif kwargs.flowerGiftN then
				poke.forme = ''
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
				poke.baseStatOverride = Tools.getTemplate(templateId).baseStats
				local spriteId = (poke.shiny and '_SHINY' or '')..(poke.side.n == 1 and '_BACK/' or '_FRONT/')..'Cherrim'
			...
			elseif kwargs.rustedSword then
				poke.forme = 'Crowned'
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
				poke.baseStatOverride = Tools.getTemplate(templateId).baseStats
				local spriteId = (poke.shiny and '_SHINY' or '')..(poke.side.n == 1 and '_BACK/' or '_FRONT/')..'Zacian-Crowned'
				poke.sprite:animMegaEvolve(Tools.getSprite(spriteId), 'Pink', 'Gray', 'Baby Blue')
			elseif kwargs.rustedShield then
				poke.forme = 'Crowned'
				local templateId = poke:getFilteredNickname() .. poke.forme
				poke.types = Tools.getTemplate(templateId).types or poke.types
				poke.baseStatOverride = Tools.getTemplate(templateId).baseStats
				local spriteId = (poke.shiny and '_SHINY' or '')..(poke.side.n == 1 and '_BACK/' or '_FRONT/')..'Zamazenta-Crowned'
				poke.sprite:animMegaEvolve(Tools.getSprite(spriteId), 'Maroon', 'Gold', 'Deep Blue')
			end
			...
			poke.typeOverride = template.types and deepcopy(template.types)
			--			
			--			poke.details = args[3]
		...
		elseif arg1 == 'faint' then
			...
			if poke.sprite.isRaid == true then
				self:message('You have defeated the Max Raid!')

				local currentBattle = _p and _p.Battle and _p.Battle.currentBattle
				if currentBattle then
					currentBattle.ended = true
					currentBattle.BattleEnded:fire()
				end
			else
			...
			if kwargs.evo and not poke.evolved then
				...
				while true do
					...
					if delete then
						...
						while not lmd do task.wait() end
		...
		elseif arg1 == 'nickname' then
			...
			if _p.gamemode == 'nuzlocke' or _p.gamemode == 'random nuzlocke' then
				self:message('Give a nickname to the ' .. poke.name .. ' you caught!')
