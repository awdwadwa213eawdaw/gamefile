return function(_p)
	...
	function PlayerData:getEtc()
		...
		etc.options = {
			...
			tSkip = options.tSkip and true or false,
			IconSFX = options.IconSFX and true or false,
			weatherEnabled = options.weatherEnabled and true or false,
			pxSetting = {
				pkmnIcon = options.pxSetting.pkmnIcon and true or false,
				itemIcon = options.pxSetting.itemIcon and true or false,
				sprite = options.pxSetting.sprite and true or false,
			},
			cHints = options.cHints and true or false,
...
--[[
	...
	function PlayerData:loadEtc(etc)
		...
		if etc.options then
			...
			options.cSpeed = etc.options.cSpeed

			for _, v in pairs({"tSkip", "IconSFX", "cHints", "weatherEnabled"}) do
				addBool(v)
			end

			options.pxSetting = {}

			for _, v in pairs({"pkmnIcon", "itemIcon", "sprite"}) do
				addBool(v, options.pxSetting)
			end

			options.currentPage = etc.options.page or 1
		end
