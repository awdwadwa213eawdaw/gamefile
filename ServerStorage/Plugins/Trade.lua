return function(_p)
	...
	local compileOffer = function(theOffer)
		local offer = ""
		for i = 1, 4 do
	...
	function Trade:getEtc(officialMyOffer, officialTheirOffer)
		if officialMyOffer ~= compileOffer(self.myOffer) or officialTheirOffer ~= compileOffer(self.theirOffer) then
			return false
		end
	...
	network:bindFunction("TradeFunction", function(sessionId, fnName, ...)
		if sessionId ~= Trade.sessionId then
			return
		end
		if not Trade[fnName] then
			if _p.debugEnabled then
