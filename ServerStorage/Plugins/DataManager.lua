...
return function(_p)
	...
	local loadingIcon = Utilities.Create 'ImageLabel' {
		...
		Image = 'rbxassetid://13045003074', -- PBallTransparent
	...
	function DataManager:loadChunk(id, withData)
		...
		if dens then
			local checkah = _p.Network:get('PDS', 'getRaidDenData', true) 
			for _, child in ipairs(dens:GetChildren()) do 
				_p.MaxRaid:GenerateRaidDen(child)
				child.Main.Transparency = .99
				child.Parent = chunk.map
			end
		end
		spawn(function()
			if not _p.Overworld.eventWeather and _p.PlayerData.isDate then	
				_p.Overworld:updateEventWeather()
			end
			if not _p.Menu.options.weatherEnabled then
				_p.Overworld:endAllWeather() --deals with everything now then
			end
			local weather = chunkData.weather
			if weather then
				if not chunkData.data.noWeather and not chunkData.indoors then
					_p.Overworld:startWeather(weather)
				end
			end
			if _p.Overworld.eventWeather then
				_p.Overworld:endWeather(_p.Overworld.eventWeather.id)
				_p.Overworld:startWeather(_p.Overworld.eventWeather.id)
				--_p.Overworld.Events:makeMarshadowPortal()
			end
		end)
		return chunk
	end
