...
if player and not script.Parent:IsA('ServerStorage') then
	...
	util.miscGui = Instance.new('ScreenGui', playerGui)
	util.miscGui.Name = 'MiscGui'
	
	util.chatBoxContainer = Instance.new('ScreenGui', playerGui)
	util.chatBoxContainer.Name = 'ChatBoxContainer'
	...
	util.guiSizeChanged = gui:GetPropertyChangedSignal("AbsoluteSize")
end


function util:layerGuis()
	local playerGui = player:WaitForChild('PlayerGui')
	util.backGui.Parent, util.gui.Parent, util.frontGui.Parent, util.miscGui.Parent, util.chatBoxContainer.Parent, util.simulatedCoreGui.Parent = nil, nil, nil, nil
	util.backGui.Parent = playerGui
	util.gui.Parent = playerGui
	util.frontGui.Parent = playerGui
	util.miscGui.Parent = playerGui
	util.chatBoxContainer.Parent = playerGui
...
function util.loopSound(id, volume, loopId)
	...
	if id == Audios.EcliseGruntBattle1 then volume = .7 end -- eclipse battle music
	local sound = util.Create 'Sound' {
		SoundId = type(id)=='number' and ('rbxassetid://'..id) or id,
		Volume = volume or .5,
		Looped = loopId==nil,
