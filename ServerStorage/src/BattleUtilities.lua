...
util.endsWith = Utilities.endsWith
...
function util.filter(tab, fn, forceArray)
	local newTab = {}
	local i = 0 -- force array is for sequential filtering
...
function util.trunc(num, bits)
	bits = bits or 0
	if bits ~= 0 then
		return bit32.extract(num, 0, bits)
	end
	return num % 2^32
end
